...
Snapshot dated 2026-05-19. The backend (`crates/cmdk/`) and app integration (`crates/app/`) are largely shipped; this doc tracks per-slice state and what's left.
...
- `CommandId` enum, currently **71 variants** across Navigation, Email, Compose, Tasks, View, Calendar, App, Undo, Smart Folders, Pinned Searches, search-index rebuild. (Started at 55; grew through calendar + select-range + smart-folder + undo + rebuild + pinned-search work.)
...
Files: `crates/cmdk/src/{id,context,descriptor}.rs`, `crates/cmdk/src/registry/{core,builders,scoring,nav,email,compose,tasks,view,calendar,app,smart_folders,usage}.rs`.
...
- `CommandArgs` enum (6 variants): `MoveToFolder`, `AddLabel`, `RemoveLabel`, `Snooze`, `NavigateToLabel`, `SmartFolderSave`. Folder selection carries `FolderId`; label-group selection carries `LabelGroupId`. The earlier dual-purpose `NavigateToFolder` arm was removed in the labels-unification slice 5 cleanup (a one-paragraph comment in `args.rs` documents the removal).
- Concrete `AppInputResolver` in `crates/app/src/command_resolver.rs` queries DB for folders and label groups.
...
- All 71 command registrations declare a default keybinding (some pass `None` - e.g. `PinnedSearchesClearAll`, `SmartFolderSave`).
...
- `CommandId::Undo` registered with `Ctrl+Z`. `is_undoable` flag on descriptors; 11 email commands marked undoable (Archive, Trash, Spam, MarkRead, Star, Pin, Mute, MoveToFolder, AddLabel, RemoveLabel, Snooze). `PermanentDelete`, `Unsubscribe`, `SelectAll`, `SelectFromHere` are intentionally non-undoable.
...
- `command_dispatch.rs`: `build_context()`, `dispatch_command()` (covers 64 commands; returns `None` for 6 parameterized + 1 unimplemented `AppAskAi`), `dispatch_parameterized()`, iced<->cmdk key conversion.
...
- `Palette` struct as a `Component` (~775 lines, `crates/app/src/ui/palette.rs`).
- Stage 1 command search with availability indicators, category badges, keybinding hints.
- Stage 2 `OptionPick` with async resolver load, generation tracking for stale-result discard, label + path breadcrumb rendering, cross-account account-name prefix.
- Stage 2 `TextInput` for `ParamDef::Text` commands (`SmartFolderSave`): the palette query field *is* the param value; Confirm submits it, no resolver call.
- Stack-based overlay with backdrop `mouse_area` for click-outside-to-close, auto-focus on open.
- Snooze preset DateTime options are built in-process by `palette.rs::snooze_preset_options()` (special-cased in `enter_snooze_stage2`), not via the resolver - the resolver has no `EmailSnooze` arm.
- `PaletteStage` is a bare unit enum with 3 variants - `CommandSearch`, `OptionPick`, `TextInput` (intentional shape, see `design.md`). Escape in either stage-2 variant returns to stage 1.
...
1. **`scroll_to_selected()`** - blocked on the iced fork not exposing `scrollable::scroll_to()`. Currently a no-op (`crates/app/src/ui/palette.rs:698`); arrow keys move the selection index but the scrollable doesn't follow when the highlight goes off-screen.
