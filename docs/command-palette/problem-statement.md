...
- **Recency** — `UsageTracker` records per-command usage counts, persisted to `command_usage.json`. Used in empty-query ordering today; folding into the fuzzy sort key is the remaining gap (see `discrepancies.md`).
