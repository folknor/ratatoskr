...
**Action-service label writes are reconciled with the recompute paths.** `confirmed_provider_label_intents` (`crates/db/src/db/queries_extra/label_intent.rs`) writes both the `thread_labels` row and the per-message `message_labels` row for every current message in the thread, in the same transaction. Removes also clear `message_keywords` rows when the label id is keyword-shaped (`kw:<keyword>`). This makes the action-success path idempotent with `recompute_thread_labels_from_messages`: a subsequent recompute (sibling-message delete, JWZ rethread, partial-delta on an unrelated message) reads the action-side rows out of the per-message tables and reproduces the same `thread_labels` set, so no provider-echo window can silently wipe the optimistic effect of a same-client label add.

**Action-service folder writes remain a separate follow-on slice.** A "move to Archive" mutation is a single-folder delta against `thread_folders` (`crates/core/src/db/queries_extra/email_actions.rs`), not a complete folder-set rewrite. The cure D layering does not cover it. The plausible answer mirrors `pending_thread_label_intents` (`docs/optimistic-label-intent.md`): a `pending_thread_folder_intents` table that the action service writes, with the read path overlaying intent on provider truth, and `threads.folder_membership_generation` bumped on confirmed provider truth so satisfied intents can be cleared. Open questions for the design slice:
