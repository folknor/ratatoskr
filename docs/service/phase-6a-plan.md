...
**2026-05-06 - mid-session shipped status (8 surfaces landed).** A run through the small / non-envelope / CRUD / kick / envelope-typed surfaces shipped:

- `calendar.set_visibility` (Task 6, plus the post-review fixup) - simple-bool shape.
- `thread_ui_state.set` (Task 5) - nullable-bool shape.
- `settings.set` (Task 4) - typed-enum-batch shape.
- `pinned_search.kick` - Drop-class notification, mirrors `gal.kick` / `calendar.kick`.
- `signature.create | update | delete | reorder` - first instance of the four-method CRUD shape.
- `contacts.group_save | group_delete` - second CRUD instance, confirmed the shape transferred without rework. The plan's three-method split (`group_create | group_update | group_delete`) collapsed to two methods because the underlying DB function is a true UPSERT and the UI always pre-generates ids.
- `account.update + account.reorder` - small UPDATE / REORDER subset of the account family.
- `account.create` - introduces the `Plaintext | Encrypted` credentials envelope (boxed because the variant body is large). Day-one wire shape so 6b's two-step OAuth flow can extend rather than redefine.

Remaining 6a scope explicitly deferred this session:

- `account.delete`. Service-side cancel-and-await needs either a new "subscribe to runner terminal completion" primitive on `SyncRuntime` / `PushRuntime` / `CalendarRuntime`, or threading body / inline / cache / search state onto `BootSharedState` so the handler can drive external-store cleanup. Current UI flow works correctly and the boot-time invariant pass cleans orphans on dirty boot, so the surface is parked rather than half-shipped.
- `internal.read_bootstrap_snapshots` + `internal.encrypt_for_storage` + `internal.decrypt_for_storage`. The bootstrap snapshot relocation requires `from_boot_ready` to become async (it currently runs sync against the read DB connection), which is a deeper boot-path refactor than the other 6a surfaces. Shipping just `encrypt/decrypt` without the bootstrap relocation does not let `rtsk::load_encryption_key` be deleted (the bootstrap path still calls it), so the bundle stays parked together until the boot path can be rebuilt.
- `draft.save` + WAL. Needs a UI-side WAL writer + a Service-side `BootPhase::DrainingDraftWAL` step. Substantial enough to want its own session; today's synchronous local SQLite write keeps the durability guarantee.
- Pinned-search per-row CRUD (`create_or_update_pinned_search`, `update_pinned_search`, `delete_pinned_search`, `delete_all_pinned_searches`, `create_smart_folder`). The plan grouped these with the kick relocation but the kick was the only one shipped; the CRUD stays UI-side. Same shape as signature CRUD when it lands.
- Pattern A re-auth token persists in `oauth.rs:149` and `state.rs:514`. These belong to the OAuth flow and are 6b territory.
- `Db::with_write_conn` / `with_write_conn_sync` / `write_db_state` deletion. Cannot land until all callers above are migrated; tracked as the closing 6a commit once the deferrals above clear.
- `docs/architecture.md` update reflecting post-6a state. Pending the final cleanup commit.
- CI lockdown script (`scripts/check_app_write_surface.sh`). Pending the final cleanup commit.

The pattern catalog established by the eight shipped surfaces is now reusable: each remaining surface inherits the per-surface checklist + per-method ack-variant rule + `WriteDbState::with_conn` idiom.
