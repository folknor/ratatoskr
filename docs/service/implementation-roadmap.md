...
**Phase 1.5 carry-forward (close out as part of Phase 2).**
- **`ReadyApp::from_boot_ready` heavy synchronous init.** Today (Phase 1.5) `crates/app/src/app.rs::from_boot_ready` opens the DB, loads stores, parses bootstrap snapshots, restores pop-out windows - all synchronously on the iced runtime thread, after `boot.ready` already returned. On a slow disk this momentarily blocks the splash transition with a frozen view. Phase 2 reworks `ActionContext` Service-side, which removes the ActionContext-side init from this critical path; relocate the body/inline/search store init to async tasks dispatched from a `BootingApp::update` arm so the splash stays responsive while they finish.
- **`apply_standard_pragmas` per-connection waste / `BootContext.db_conn` unused.** Phase 1.5 holds an idle DB connection in `BootContext` past boot, ready for Phase 2's `ActionContext` to consume. Three connections to the same WAL DB (Service idle + UI read + UI write) is fine but wasteful; if Phase 2 slips, the idle connection is a long-lived resource leak that scales with respawn count. Either the Phase 2 PR consumes it as planned, or `BootContext` drops the field and Phase 2 reopens.
- **`SchemaVersionChanged` end-to-end test (`--test-fake-schema=N`).** The mismatch path in `service_client.rs::respawn` is unit-tested via the Display contract, but the full subprocess path - where the schema actually changes across a respawn - is not. Add a `--test-fake-schema=N` flag analogous to the existing `--test-fake-version`, then a real-subprocess test that flips the value across SIGKILL and asserts `ClientError::SchemaVersionChanged` arrives via Terminal. Phase 2 introduces real schema-version sensitivity (the action service depends on the schema being what the UI thinks it is), so the test naturally lands here.
- **`from_boot_ready` re-loads the encryption key the Service already validated.** Phase 1.5's `crates/app/src/app.rs:241` calls `rtsk::load_encryption_key` to re-read the file the Service has already loaded into `BootContext::encryption_key`. Both sides run the TOCTOU permission repair via `fchmod`; a key file that changes between the two reads (permission race, hostile rewrite, FS glitch) lands the UI on a different key than the Service. Phase 2 plumbs `BootContext::encryption_key` through the IPC boundary so the UI consumes the Service's already-validated key. Hard requirement, not a nice-to-have - flagged by `arch` review (claude).
- **Pre-dispatch generation-counter bumps for the action pipeline.** `crates/app/src/handlers/commands.rs::dispatch_plan` sends the execution plan to `batch_execute` without bumping `nav_generation` or `thread_generation`. Some actions optimistically mutate visible thread state before dispatch; an older `ThreadsLoaded` or `NavigationLoaded` result landing after the mutation can overwrite it with a stale snapshot. The Phase 2 plan's "Generation counters bump pre-dispatch on plan submission" item must be treated as a checklist item against `dispatch_plan` and the related compose-send / undo / snooze paths, not just `batch_execute`. Flagged by `bugs` review (codex).
- **`parent_death` crate boundary.** `crates/app/src/service_client.rs` reaches into `service::parent_death::*` for `ProcessGuard`. App → Service even though Service is conceptually the embedded child. Phase 2's `service-state` crate split is the natural moment to extract parent-death machinery into a `process-lifetime` micro-crate consumed by both `service` (for the in-child `exit_if_parent_missing`) and `app` (for `ProcessGuard`). Aesthetic only - practical cost in the single-binary build is zero. Flagged by `arch` review (claude).
- **`boot_progress::emit` per-phase regression test.** `crates/service/src/boot_progress.rs:36-55`'s doc-comment names the contract ("`OUTBOUND_QUEUE_CAP=1024` must remain >> Phase-1.5 boot frame count") but no test enforces it. The ordering test at `tests/dispatch_in_process.rs:516-584` only exercises a single-migration fresh DB (one `Migrating` frame). Phase 2 must extend the contract: any new boot phase or chatty `MustDeliver` notification ships with a regression test bounding total emit count, or a class-aware emit helper that picks `try_send` vs `send` based on `NotificationClass`. Flagged by both `arch` and `bugs` reviews.
- **`BootSharedState` flood resilience for `boot.ready`.** `crates/service/src/handlers/boot.rs` parks each `boot.ready` request on the shared `Notify` and consumes a `JoinSet` slot until boot completes. Stdio is private so a malicious flood is not realistic, but a UI bug that re-issues `boot.ready` would balloon `JoinSet` linearly. Cheap fix: an `AtomicBool` "already in flight" so subsequent callers either join the existing one or fail fast. Land in the Phase 2 plan since `boot.ready`'s shape is being touched there. Flagged by `arch` review (claude).
...
- **Race `spawn_inner`'s `health.ping` against direct child-exit observation.** Phase 1.5 carry-forward, flagged by `bugs` review (claude+codex). Today `ServiceClient::spawn_inner` sends `health.ping` with a 5 s timeout; if the child has already exited (e.g., boot's `LoadingKey` hit a missing key file), the reader_task's EOF detection drops the pending request and returns `ServiceCrashed` quickly under typical load. Under heavy parallel-test scheduling pressure or under a real production overload, the reader_task can be starved long enough that the 5 s ceiling becomes the actual wall time before the request fails. The fix: spawn a background task that awaits `child.wait()` once and broadcasts the result via `tokio::sync::watch`/oneshot; `tokio::select!` between the ping future and the child-exit signal in `spawn_inner` (and analogously in the `boot.ready` path with its 600 s timeout). On child-exit-first, jump straight to `elevate_initial_boot_error` without waiting for the ping ceiling. Phase 1.5 ships with two pragmatic mitigations - elevate's abort-handle joins run concurrently via `tokio::join!` (200 ms total instead of 600 ms) and `wait_with_kill_watchdog` shrunk from 2 s to 1 s, plus the no-key tests use an overall-deadline event-collector helper - but the structural fix belongs here where the rest of the respawn polish lands.
