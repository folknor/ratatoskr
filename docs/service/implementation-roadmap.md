...
- **DEFERRED to Phase 8: `boot_progress::emit` per-phase regression test / class-aware helper.** First implementation attempt (try_send for Coalesce/Drop, awaited send for MustDeliver) introduced a hang in the `service_subprocess` test cohort and was reverted. See Phase 8 "Two `service_subprocess` tests still flaky" for the investigation surface. Coalesce-class try_send remains correct for boot.progress / sync.progress; the MustDeliver-side awaited-send path needs a proper investigation of writer-task drain ordering before re-attempting.
- **DEFERRED to a future Phase 2 follow-up: `ReadyApp::from_boot_ready` heavy synchronous init.** Body / inline / search store init still runs synchronously inside `from_boot_ready`. Less load-bearing now that `ActionContext` is Service-side, but the splash-blocking moment remains on slow disks. Pure UI surgery; can land in any later phase.
- **DEFERRED to a future Phase 2 follow-up: `SchemaVersionChanged` e2e test (`--test-fake-schema=N`).** No real-subprocess test flips the schema across a respawn yet. Phase 2 introduced real schema-version sensitivity (the action worker depends on the schema being what the UI thinks it is), so the test still belongs here in spirit; it just didn't land in the same milestone.
- **DEFERRED to a future Phase 2 follow-up: `from_boot_ready` re-loads the encryption key.** UI still re-reads `ratatoskr.key` post-handshake instead of consuming the Service's already-validated handle. The TOCTOU window the arch review flagged remains. The fix is an `internal.encrypt_for_storage` IPC method (or a one-shot key-export); both shapes are sketched in `phase-2-plan.md` § 19d.
