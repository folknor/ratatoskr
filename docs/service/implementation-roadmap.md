...
**In scope.**
- **Respawn loop.** If the Service exits unexpectedly: `ServiceClient` awaits the dying child's `wait()` (so the file lock is genuinely released) before calling `Command::spawn()`. Without that wait, the new Service races the old one for the lock and bounces with `AnotherInstanceRunning`, which is indistinguishable from a real second-instance attempt. No backoff, no crashloop detection, no UI status indicator yet (Phase 8). Pending requests at crash time fail with `ClientError::ServiceCrashed`. UI surfaces this per call site (logged today; toast UX in Phase 8).
...
- **First-ping timeout extension.** Phase 1's default 5 s timeout becomes load-bearing once schema migrations run during boot. A 50 GB DB migration can take 60 s+. Extend the first ping timeout to 10 minutes, and have the Service emit `boot.progress` `Coalesce` notifications during long migrations so the UI splash can render progress rather than appearing frozen. The chicken-and-egg here (notifications need a consumer; the consumer is App; App is awaiting the boot handshake) is real and the Phase 1.5 plan must pick one of: (a) a small pre-iced splash, (b) `ServiceClient` buffers `boot.progress` notifications and replays once App subscribes, or (c) App boots into a "Loading" state that consumes notifications and renders the splash, then transitions. Default proposal: (b), since it doesn't restructure App.
- **Single-instance lock.** Service takes an OS-level file lock on the app data directory at boot. If locked, exit with the dedicated boot-error exit code (see below); UI surfaces "Ratatoskr is already running." `AnotherInstanceRunning` is *not* a JSON-RPC `ServiceError` - the Service exits before stdio is established as a JSON-RPC channel, so the UI must distinguish exit codes (clean / handshake-failure / instance-already-running / migration-failure). Pick a small `BootExitCode` enum and document the mapping. Cross-platform file-lock library: choose between `fs2` and `fd-lock` in the Phase 1.5 plan; both work on Linux and Windows but differ in WSL behavior and lock-file vs file-handle ergonomics.
