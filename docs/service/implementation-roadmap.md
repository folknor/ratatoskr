...
**Phase 6d - contacts pipeline + strict transitive lockdown.** Plan: `docs/service/phase-6d-plan.md`.
- **6d-A - Contacts pipeline relocation.** Service-side `contacts.contact_save_with_writeback` (extends today's local-only `contacts.contact_save` with provider write-back) + new `contacts.contact_delete` IPC. UI handlers `handle_save_contact` / `handle_delete_contact` route through the IPC; `service::actions::contacts::*` direct calls disappear from `app/src/`. The `app.action_ctx` field, the `ReadyApp::action_ctx()` accessor, the `Db::phase_6c_pending_write_state` accessor, and the UI-side `rtsk::load_encryption_key` call are all deleted. Bulk-import path remains local-only (renamed `save_contact_local_only` to make the asymmetry explicit). Closes the residual UI write surface architecture.md § "Current Exceptions" tracks.
- **6d-B - Strict transitive lockdown.** Four parallel Cargo edge breaks:
  - **`common -> service-state`**: extract `SyncProviderCtx` and the sync trait surface (`sync_initial` / `sync_delta`) into a new `provider-sync` crate. `common::ProviderOps` shrinks to action / folder / send / draft / attachment / profile / connection methods. `common/Cargo.toml` drops `service-state`.
  - **`{gmail,jmap,graph,imap} -> service-state`**: each provider's `ops.rs` loses the two sync method bodies; they migrate to `provider-sync/src/<provider>.rs` as orphan-impls of `ProviderSyncOps`. The four provider Cargo.tomls drop `service-state`.
  - **`sync -> service-state`** (transitively via `rtsk`): split `sync` into `sync-logic` (pure: `bundling`, `filters`, `smart_labels`, `threading`, `config`, plus the two `pipeline::clear_*` helpers) + `sync` (Service-side persistence, keeps `service-state`). `rtsk` re-exports point at `sync-logic`; `core/Cargo.toml` drops `sync`.
  - **Provider registry shape**: post-6d-B the per-account registry returns `ProviderHandle { ops: Arc<dyn ProviderOps>, sync_ops: Arc<dyn ProviderSyncOps> }`; both Arcs may wrap the same client.
- **6d-C - Lockdown test.** `crates/service-state/tests/lockdown.rs` gains `app_crate_must_not_transitively_depend_on_service_state` (BFS from `app`, target = `service-state`, blessed = `["service"]`). The 6c-11 cal-target test stays as a regression guard against a different shape of mistake.
