...
- **Contacts pipeline.** `service::actions::contacts::{save_contact, delete_contact}` still runs UI-side via the `action_ctx` field on `ReadyApp` (uses `ReadDbState`, not `WriteDbState`, so 6b/6c-11's lockdowns hold). Migration to a `contacts.contact_save_remote` / `contacts.contact_delete` IPC pair tracked for Phase 6d. The `Db::phase_6c_pending_write_state` accessor stays until contacts migrate.
- **`service-state` strict transitive lockdown.** `app -> rtsk -> common -> service-state` is a structural path because `common::types::ProviderSyncContext` is parameterised over `&WriteDbState` / `&BodyStoreWriteState` / etc. The cal-out-of-app lockdown closes the regression class Phase 6c targeted; the broader `app -> service-state` blackout requires a separate refactor (move provider sync types out of `common`) tracked for Phase 6d/8.
