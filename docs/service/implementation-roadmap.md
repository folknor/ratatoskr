...
**Phase 6c - calendar event mutations.** Plan: `docs/service/phase-6c-plan.md`. **LANDED** (2026-05-06; see commits tagged `phase 6c-N:`). Three flat operations (create / update / delete via `cal::actions::*`) relocated Service-side via a sibling `CalendarActionContext` (writer-half + encryption key) and the `cal_action.execute_plan` IPC. The `action_jobs` journal kind CHECK widened to include `'calendar_plan'`; the worker dispatches per-leased-op based on parent kind, emitting `CalendarOperationOutcome` per op + `CalendarActionCompleted` per plan. UI awaits via `ServiceClient::pending_calendar_actions` (latch pattern mirroring Phase 5's `pending_calendars`). `cal` dropped from `app/Cargo.toml`; transitive lockdown (`crates/service-state/tests/lockdown.rs`) asserts `app` cannot reach `cal` through any path-dep chain.

**Deferrals tracked from 6c:**
- **Contacts pipeline.** `service::actions::contacts::{save_contact, delete_contact}` still runs UI-side via the `action_ctx` field on `ReadyApp` (uses `ReadDbState`, not `WriteDbState`, so 6b/6c-11's lockdowns hold). Migration to a `contacts.contact_save_with_writeback` / `contacts.contact_delete` IPC pair tracked for Phase 6d (plan: `docs/service/phase-6d-plan.md`). The `Db::phase_6c_pending_write_state` accessor stays until contacts migrate.
- **`service-state` strict transitive lockdown.** `app -> rtsk -> common -> service-state` is a structural path because `common::types::SyncProviderCtx` is parameterised over `&WriteDbState` / `&BodyStoreWriteState` / etc. Three additional structural paths exist that the original framing missed: `app -> rtsk -> sync -> service-state` and `app -> rtsk -> {gmail,jmap,graph,imap} -> service-state` (the four provider crates impl `ProviderOps::sync_initial`/`sync_delta` and pull the dep directly). The cal-out-of-app lockdown closes the regression class Phase 6c targeted; the broader `app -> service-state` blackout requires four parallel edge breaks (move sync types out of `common`, split `sync` into pure-logic + persistence halves, extract a `provider-sync` orphan-impl crate so the four provider crates drop the dep) tracked for Phase 6d (plan: `docs/service/phase-6d-plan.md`).
- **`boot_progress_notifications_emitted_in_order`** test (`crates/service/tests/dispatch_in_process.rs`) marked `#[ignore]` due to a workspace-level hang post-Phase-6c-7 / 6c-9 that does not reproduce per-package. Phase 8 triage.
- **RSVP semantics.** Out of scope. `WireCalendarOperation::RespondToInvite { event_id, response, comment, send_response }` variant lands when the calendar editor grows RSVP UI.
- **Series-vs-occurrence semantics.** Out of scope. `EventScope` enum (`Single`, `Occurrence(date)`, `Series(from_date)`) lands when the editor grows recurrence-edit UI.

**Phase 6d - contacts pipeline + partial structural lockdown.** Plan: `docs/service/phase-6d-plan.md`. **LANDED** (2026-05-06; see commits tagged `phase 6d-A:` and `phase 6d-B:`).

- **6d-A - Contacts pipeline relocation.** Service-side `contacts.contact_save_with_writeback` (extends today's local-only `contacts.contact_save` with provider write-back) + new `contacts.contact_delete` IPC. UI handlers `handle_save_contact` / `handle_delete_contact` route through the IPC; `service::actions::contacts::*` direct calls disappear from `app/src/`. The `app.action_ctx` field, the `ReadyApp::action_ctx()` accessor, the `Db::phase_6c_pending_write_state` accessor, and the UI-side `rtsk::load_encryption_key` call all deleted. Bulk-import path remains local-only (renamed `save_contact_local_only` to make the asymmetry explicit). Closes the residual UI write surface architecture.md § "Current Exceptions" tracked. `scripts/check_app_write_surface.sh` adds `phase_6c_pending_write_state` to its FORBIDDEN_PATTERNS so the symbol cannot return.
- **6d-B - `SyncProviderCtx` + `ProviderSyncOps` carve-out.** New `crates/provider-sync/` crate hosts the trait + ctx; per-provider orphan-impls (`gmail_impl.rs`, `jmap_impl.rs`, `graph_impl.rs`, `imap_impl.rs`) delegate into each provider's existing `sync::*` entry-points. `common::ProviderOps` shrinks to the action / send / draft / folder / profile / connection methods - none of which need writer-half handles. `common/Cargo.toml` drops `service-state` and `tokio-util`. `service::actions::provider::create_provider` returns `Box<dyn ProviderSyncOps>` (the trait inherits `ProviderOps`, so action callers continue calling `provider.archive(...)` etc. via supertrait method resolution; no upcast needed). `graph::sync::graph_initial_sync` / `graph_delta_sync` flatten their parameter list to take individual writer-half handles instead of `&SyncProviderCtx` - required to avoid a `provider-sync -> graph -> provider-sync` Cargo cycle.

**Scope reduction discovered during 6d-B implementation.** First-pass drafting assumed the four `app -> ... -> {gmail,jmap,graph,imap} -> service-state` edges were closeable via the orphan-impl move + a Cargo dep drop on each provider. Implementation discovered that each provider's `sync_*` body uses writer-half types (`WriteDbState`, `BodyStoreWriteState`, `InlineImageStoreWriteState`, `SearchWriteHandle`) directly throughout its persistence layer; closing the edge would require relocating the entire provider sync subtree out of the provider crate - a much larger refactor than 6d-B's trait-surface move. The `app -> ... -> sync -> service-state` edge has the same shape (`sync::persistence` is the crate's reason to depend on service-state). 6d-B closes the `common` edge only; the four provider edges and the `sync` edge remain open and are deferred to Phase 8 alongside the rest of the structural lockdown work. The strict-transitive lockdown test (originally planned as 6d-C) is dropped from 6d - blessing the four provider crates + `sync` would weaken the test below the value of writing it. The Phase 6b direct-dep test still rules out `use service_state::*` in app source; Phase 6c-11's `app -> cal` transitive test still catches the cal-action regression class.

**Phase 6d carry-forward to Phase 8:**
- **`{gmail,jmap,graph,imap} -> service-state` Cargo edges.** Closing them requires relocating each provider's sync subtree (`crates/{provider}/src/sync/`) out of the provider crate - either into `provider-sync` (one big crate that knows every provider's persistence) or into per-provider `*-sync` sibling crates. Either path is multi-week structural work and was reclassified to Phase 8 once 6d-B's trait-surface move surfaced the actual cost. The four edges are not UI-reachable for writes today (Phase 6b's direct-dep test prevents `use service_state::*` in app source) but the Cargo graph still permits the structural reachability.
- **`sync -> service-state` edge** (transitively via `rtsk -> sync`). Same shape as the provider edges: `sync::persistence` writes through the four writer halves. Closing the edge requires either splitting `sync` into pure-logic + persistence halves or moving the entire crate Service-side. Phase 8.
- **Strict transitive lockdown test.** When the four provider edges + the sync edge close, `crates/service-state/tests/lockdown.rs` gains `app_crate_must_not_transitively_depend_on_service_state` (BFS from `app`, target `service-state`, blessed `["service"]`). Until then, the test would have to bless five additional crates and is not meaningfully "strict."
...
**Phase 7 status (as landed).** Plan: `docs/service/phase-7-plan.md`. **LANDED** (2026-05-07; see commits tagged `phase 7-N:` or `phase 7-Na:`/`phase 7-Nb:`/etc. for the sub-split slices). Of the entries above:
- ✓ **Schema + AttachmentCacheInfo + version sentinel (7-1).** `attachments.text_indexed_at` column + partial index, `attachment_extracted_text` table, both extending the v100 in-place schema with no new migration row. `INDEX_SCHEMA_VERSION = 2` constant. `check_schema_version_and_dispatch` stub lands and the post-7-9c dispatcher consumes the flag.
- ✓ **`text_extract` module + extractors + fixture corpus (7-2a/b/c).** `text_extract::{mod,pdf,ooxml,plain}` as pure functions. Two-cap policy (`MAX_INPUT_BYTES = 50 MB`, `MAX_EXTRACTED_TEXT_BYTES = 100 KB`). PDF: `/Encrypt` head-inspection pre-flight (first 64 KB + last 4 KB) before `pdf-extract`. OOXML: two-layer zip-bomb defense (CD size cap + per-entry `Read::take`). Plain: `encoding_rs` BOM-detect + UTF-8 / Windows-1252 fallback + control-char-ratio guard. Synthetic byte-literal + zip-built fixtures inline; real-world `.pdf`/`.docx` corpus deferred.
- ✓ **Tantivy schema fields + per-attachment `add_text` (7-3a).** Four schema fields (`attachment_text`, `attachment_filename`, `attachment_mime`, `attachment_id`) + 32-token `rtskbnd` boundary pad inserted before all-but-the-first attachment_text value. Default Tantivy `POSITION_GAP=1` already blocks `slop=0` cross-attachment phrase matches; the pad is belt-and-suspenders against `slop>=2`. `INDEX_SCHEMA_VERSION` bumped 1->2. `SearchDocument.attachments: Vec<AttachmentDocFragment>` replaces the original three parallel fields. Provider crates pass thin docs (`attachments: Vec::new()`); `ExtractRuntime` populates via DB at extraction time.
- ✓ **`ExtractRuntime` + IPC wire types + drain integration + panic supervisor (7-4a/b/c/d).** `attachment_lock::SWEEP_LOCK` relocation. `service-api/src/extract.rs` wire types with `service_generation` on every state-changing notification; four `Notification` variants, `ClientNotification::ExtractBackfillKick`, two `RequestParams` variants. `ExtractRuntime` mirrors `CalendarRuntime` shape with `Arc<Mutex<HashSet<content_hash>>>` enqueue dedupe + `spawn_blocking + tokio::time::timeout(30s)` per-item dispatch + panic supervisor (per-item `tokio::spawn` so JoinError captures panics). The 7-4d production producer landed in two halves (initial scaffold + post-7-8 revival): `spawn_post_ready_extract_startup` constructs the runtime after `wait_for_ready` and installs it on `BootSharedState`. The deadlock that bricked the first revival attempt was a `SearchWriteHandle` clone leaking past writer-task EOF; fix is the single-use `search_write` slot consumed by either `take_search_write` from inside the producer on success or defensively from `run_shutdown_drain` before the search-writer await.
- ✓ **Drain order extension.** `Push -> Calendar -> Sync -> Extract -> search-writer -> sentinel`, with `Rebuild` slot inserted between `Extract` and `search-writer` post-7-9a. `ExtractRuntime::shutdown()` is async (cancels `CancellationToken`, awaits stored worker `JoinHandle`); rebuild task drain calls `take_rebuild_task -> cancel -> abort -> await`.
- ✓ **`attachment.fetch` cache-miss + cache-hit hooks (7-5).** `should_enqueue_extraction(status: Option<&str>)` is the gate; pinning the schema's status taxonomy. Defensive no-op when `extract_runtime()` returns None (relevant only during the boot window before the post-ready producer fires).
- ✓ **Post-boot backfill (event-driven) + UI fan-out (7-6).** `find_unindexed_cached_attachments` partial-index SELECT + `handle_backfill_kick` enqueue. Two trigger paths: one-shot from `Message::ServiceBootReady` and an hourly `iced::time::every` ticker (1-h cadence; missed kicks self-heal on the next hour because the work is idempotent). Drop-class kick semantics; thundering-herd bound by the `LIMIT 1000` per kick + `ExtractRuntime` semaphore.
- ✓ **Re-index propagation (7-7).** Worker emits `WriterCommand::Index` with full `Vec<AttachmentDocFragment>` populated from DB at extraction time via `db::queries_extra::extract_reindex` (`find_message_ids_referencing_content_hash`, `select_messages_for_index_batch`, `select_attachment_fragments_batch`). `ExtractProgress` per drained item; `ExtractCompleted` when `in_flight_hashes` clears.
- ✓ **Search-result disambiguation + per-attachment attribution (7-8).** Top-N -> single batched DB SELECT joining `attachments` + `attachment_extracted_text` -> per-attachment `SnippetGenerator` against each segment. `match_kind: MatchKind` (`Body` / `Subject` / `From` / `Attachment { attachment_id, filename, mime, snippet }`) plus `also_matched: Vec<MatchKind>` (above 50%-of-top-score threshold, score-descending). Tiebreak: term frequency, then filename alphabetical. UI renders the "matched in *<filename>*" annotation in the result-row card.
- ✓ **`index.rebuild` IPC + palette command + schema-version dispatch (7-9a/c/d).** Wipe-only path landed. `handle_rebuild` mints a UUIDv4 rebuild_id, rejects concurrent rebuilds via `boot_state.rebuild_in_flight_id()` (or pre-empts when `force=true`), spawns `service::rebuild::run_wipe_rebuild`, installs `RebuildTaskState` on `BootSharedState`, acks the IPC. The task sends `WriterCommand::Clear`, runs `reset_extracted_text_for_rebuild`, iterates every message in 200-row chunks, builds `SearchDocument`s in parallel, sends per-chunk `Index` commands, emits `IndexRebuildProgress` (Coalesce by rebuild_id) per chunk + `IndexRebuildCompleted` (MustDeliver) at the end, then triggers `extract.backfill_kick` so attachment text re-extracts. Schema-version mismatch dispatcher (`spawn_post_ready_schema_rebuild`) clears `pending_schema_rebuild`, calls `handle_rebuild`, polls `rebuild_in_flight_id` until clear, then writes `.version`. Sentinel-write ordering preserved: a mid-rebuild crash leaves the OLD `.version` on disk and the next boot re-fires. Palette command `CommandId::AppRebuildSearchIndex` ("Rebuild Search Index") routes through the same handler with `Wipe` + `force=false`.
- ✓ **Per-package `brokkr check -p service` clean across all landed commits.** Workspace-wide `brokkr check` continues to trip the pre-existing `boot_progress_notifications_emitted_in_order` ignored-but-running hang (Phase 6c carry-forward owned by Phase 8); per-package runs are not affected.

**Phase 7 known-gaps.** Phase 8 carries forward:
- **Integration test cohort deferred to Phase 8.** The plan called for `crates/service/tests/extract_in_process.rs` to cover end-to-end fetch -> extract -> re-index -> search annotation, status-aware idempotency, eviction-during-extract, cross-attachment phrase non-match, body+attachment co-match, backfill-kick semantics, and rebuild cancellation. None landed - the in-process harness needs the same account-seeding + same-data-dir respawn + action-notification-reading fixtures Phase 8 already owns for the Phase 2 T1 cohort and the flaky `service_subprocess` re-enable. Lifecycle unit tests for `ExtractRuntime` (enqueue-after-shutdown, concurrent-same-hash dedup) landed in 7-4c. Manual-test-matrix entries 11-14 cover the user-visible flows in the meantime.
- **PreserveExisting dual-index path.** v1 ships Wipe-only; the plan-doc § "phase 7-9 - DEFERRED" calls out the rework: `SearchWriteHandle` is moved at construction (into `SyncRuntime`) rather than consulted via `boot_state`, so opening `<search_index_next>/` adjacent with a parallel writer + atomic directory swap + UI reader rebind requires plumbing rework. Re-introduce when index size + rebuild duration become user-pain (today's seeded mailbox rebuilds in seconds). Manual-test-matrix entry 13 covers the "search-briefly-unavailable" Wipe UX.
- **Status-bar visual surface for rebuild progress.** The `IndexRebuildProgress` / `IndexRebuildCompleted` notification arms in `update.rs` log at `info` level only. Future work: `Option<RebuildProgressState>` field on `ReadyApp`, render in `status_bar.rs` alongside the per-account sync progress. The wire path is verified end-to-end via logs; the UI surface is purely cosmetic.
- **Reader rebind on `IndexRebuildCompleted`.** Today the UI keeps the stale `SearchReadState` reader handle until the next app launch. The plan called for `SearchReadState::init` re-running on completion so the new index is reachable in-session. Lands as a small UI change once the visual surface above is in flight.
- **`local_drafts` re-emit during Wipe rebuild.** `run_wipe_rebuild` iterates `messages` only; `local_drafts` rows are in the search index pre-rebuild and would need to be re-emitted explicitly. Today they vanish from the index until a draft auto-save touches them. Acceptable v1 because draft rows are rare and the next save round-trips them; flag for cleanup with the visual surface.
- **Real-world fixture corpus.** 7-2 ships synthetic byte-literal + zip-built fixtures inline; the plan's checked-in `.pdf` / `.docx` / `.xlsx` / `.pptx` corpus + the malicious zip-bomb `.docx` are deferred to the integration cohort. Without the corpus the per-extractor unit tests cover the dispatch + cap behavior but not the real-PDF-with-CMap-CJK class of regressions; ship for Phase 8 alongside the integration tests.
- **`attachment_extracted_text` orphan sweep.** The cross-store invariant pass (`startup_invariants.rs`) does not include `attachment_extracted_text` in its orphan iteration. Worst-case 100 KB per orphan content_hash; typical 1000-msg/day mailbox with 5% attachment turnover ~150 orphans/year ~15 MB/year accumulation. Phase 8 owns the extension as part of the cross-store invariant pass optimization work already on its plate.
- **`extract.cancel` IPC.** Deferred from the plan. Rebuild cancellation is covered via the drain path; per-attachment user-cancellable extraction has no UI consumer today and was dropped per CLAUDE.md no-speculative-features.
...
- **Phase 7 integration test cohort (`extract_in_process.rs`).** Phase 7 carry-forward. The plan called for end-to-end fetch -> extract -> re-index -> search annotation, status-aware idempotency, eviction-during-extract, cross-attachment phrase non-match (position-gap working), body+attachment co-match, backfill-kick semantics, and rebuild cancellation. None landed - they share the same harness need with the Phase 2 T1 cohort and the flaky `service_subprocess` re-enable (account-seeding + same-data-dir respawn + action-notification-reading). Lands here once the harness extension is in flight; lifecycle unit tests for `ExtractRuntime` already cover the in-process surface today (enqueue-after-shutdown, concurrent-same-hash dedup), and manual-test-matrix entries 11-14 cover the user-visible flows in the meantime.
- **Phase 7 PreserveExisting dual-index path.** Phase 7 carry-forward. v1 ships Wipe-only - search is briefly unavailable while a rebuild runs. The plan-doc § "phase 7-9 - DEFERRED" calls out the rework: `SearchWriteHandle` is moved at construction (into `SyncRuntime`) rather than consulted via `boot_state`, so opening `<search_index_next>/` adjacent with a parallel writer + atomic directory swap + UI reader rebind requires plumbing rework. Re-introduce when index size + rebuild duration become user-pain (today's seeded mailbox rebuilds in seconds; a 50 GB / 70k-thread mailbox is the projection where the gap becomes user-visible).
- **Phase 7 status-bar visual surface for rebuild progress + reader rebind on completion.** Phase 7 carry-forward. The `IndexRebuildProgress` / `IndexRebuildCompleted` notification arms in `update.rs` log at `info` level only; future work stores `Option<RebuildProgressState>` on `ReadyApp` and renders in `status_bar.rs`. Same-PR work: re-run `SearchReadState::init` on `IndexRebuildCompleted` so the new index is reachable in-session (today the UI keeps the stale reader handle until the next app launch). Both are pure UI surgery; the wire path is verified end-to-end via logs.
- **Phase 7 `local_drafts` re-emit during Wipe rebuild.** Phase 7 carry-forward. `run_wipe_rebuild` iterates `messages` only; `local_drafts` rows that were in the search index pre-rebuild vanish until a draft auto-save touches them. Acceptable v1 because draft rows are rare and the next save round-trips them; flag for cleanup alongside the visual surface.
- **Phase 7 real-world fixture corpus.** Phase 7 carry-forward. `text_extract` ships synthetic byte-literal + zip-built fixtures inline; the plan's checked-in `.pdf` / `.docx` / `.xlsx` / `.pptx` corpus + the malicious zip-bomb `.docx` are deferred. Without the corpus the per-extractor unit tests cover the dispatch + cap behavior but not the real-PDF-with-CMap-CJK class of regressions. Lands alongside the integration tests above.
- **Phase 7 `attachment_extracted_text` orphan sweep.** Phase 7 carry-forward. The cross-store invariant pass (`startup_invariants.rs`) does not include `attachment_extracted_text` in its orphan iteration. Worst-case 100 KB per orphan content_hash; typical 1000-msg/day mailbox with 5% attachment turnover ~150 orphans/year ~15 MB/year accumulation. Folds into the cross-store invariant pass optimization above (same per-account scan loop).
