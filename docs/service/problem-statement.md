...
- ✓ `WriteDbState` flip at the Service boundary. `cal::sync::calendar_sync_account_impl` takes `&service_state::WriteDbState` and returns a `CalendarSyncOutcome { mutated, result }`. Body derives a `ReadDbState` view internally so deeper helpers (still on `&ReadDbState::with_conn` for writes) compile unchanged - that escape moves into a Phase 6 cleanup alongside `cal::actions`'s relocation. The Service-facing surface is honest about needing write capability and about whether a partial commit happened (drives `CalendarChanged` emission - see "CalendarChanged emission" below).
