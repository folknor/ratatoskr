...
**Start.** UI launches; UI spawns the Service as a child process via `tokio::process::Command`. UI passes the app data directory and runtime config via command-line args. The Service handshake is two-phase from Phase 1.5 onward: the UI's first `health.ping` carries `PROTOCOL_VERSION` for version verification (mismatch with the UI's compile-time constant is a fatal boot error); the Service then runs the slow boot sequence (single-instance lock + key load + DB open + schema migrations + pending-ops recovery + queued-drafts sweep + thread-participants backfill, emitting `boot.progress` notifications throughout) and finally answers `boot.ready` with `{ ready, schema_version, migrations_applied }`. The UI splits its spawn flow on the same boundary: `SpawnEvent::ChildSpawned` fires after the version-check ping (the App can subscribe to notifications), `SpawnEvent::BootReady` fires after the slow boot returns (the App transitions Booting -> Ready).

**Health.** Service heartbeats every 30 s. The UI distinguishes `health.ping` `Timeout` (transient; do not respawn - long migrations otherwise produce a tight respawn loop) from hard errors (the writer task died, reader EOF, etc.; respawn). Minimal respawn (cold restart, no in-flight replay) lands in Phase 1.5 so any Service crash during Phases 2-7 doesn't take the app down. Each Service incarnation gets a `service_generation: u32` tag (UI-side counter, bumped on every respawn); the reader task tags every notification with its captured generation at enqueue time, and the dispatch side drops notifications whose tag does not match the live generation. Closes the cross-respawn race where a dying-but-still-flushing reader could land an old `BootProgress` (or, in Phase 2+, a stale `action.completed`) into the new incarnation's UI state. Polish (backoff, crashloop detection, UI status indicator) lands in Phase 8.
...
- **Linux**: `pre_exec` hook calling `prctl(PR_SET_PDEATHSIG, SIGTERM)`. After the hook runs, the Service code re-checks `getppid()` once at startup; if `getppid() == 1` the parent already died before the hook took effect, exit immediately. The recheck runs as the first synchronous statement in `run_service_blocking` - before logger init, panic hook, runtime build - so the fork-to-recheck window is as small as a `getppid` syscall.
- **Windows**: a Job Object with `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE` is the documented Windows idiom for child-lifetime binding. The UI creates the Job before spawning the Service, sets the kill-on-close flag, and calls `AssignProcessToJobObject` immediately after `Command::spawn()` returns. When the UI dies (handle released), the OS terminates every process in the Job. Side benefit: any further grandchildren the Service spawns (e.g. PDF/OOXML extractor subprocesses in Phase 7) inherit the Job and are killed with the parent.

  There is a small window between `spawn()` returning and `AssignProcessToJobObject` running where the Service is not yet in the Job; if the parent dies in that window the Service can outlive it. **Accepted residual risk in v1.** Tightening this would require dropping `tokio::process::Command` for raw `CreateProcess` with `CREATE_SUSPENDED` + assign + `ResumeThread`, since `Command` doesn't expose process-state-on-spawn control. The PID-reuse race that an `OpenProcess(parent_pid)` lookup would have is avoided here because the parent's `Child` handle keeps the PID stable until assignment.

PID-polling is rejected: PID reuse is fast on busy systems, and the Service would attach to a stranger if the parent died between polls. `OpenProcess(parent_pid)` after Service startup has the same flaw on Windows (TOCTOU between exec and OpenProcess) and is also rejected; the Job Object pattern bounds the race to spawn->assign instead.
...
The lock is *not* a JSON-RPC `ServiceError`: the Service exits before stdio is established as a JSON-RPC channel. The UI distinguishes Service exit codes via the `BootExitCode` enum: `HandshakeFailure = 70`, `AnotherInstanceRunning = 71`, `MigrationFailure = 72`, `KeyLoadFailure = 73`. Values are picked outside the clap (=2), Rust panic (=101), and shell-signal (137 / 143) ranges so `wait().status.code()` cannot ambiguously confuse a real boot failure with a runtime-induced exit. **Terminal-failure policy:** all four `BootExitCode` variants are terminal at boot time - the UI logs the cause, surfaces a fatal error, and `iced::exit()`s rather than respawning. Without this, a missing key file would respawn one Service per second forever (per-PID log files would accumulate at ~86k per day). Only "Service was running and crashed" (post-`BootReady` reader EOF or heartbeat hard-error) triggers a respawn attempt. The Phase 1.5 file-lock library is `fs2` (chosen over `fd-lock` for ecosystem reach; both work on Linux and Windows).
...
| Calendar provider sync writes | UI-side via `handlers/provider.rs` | Phase 5 - LANDED. `CalendarRuntime` runs Service-side; UI fires `calendar.kick` on `Message::SyncTick`. `cal::sync::calendar_sync_account_impl` flipped to `&service_state::WriteDbState`. Calendar event mutations (`cal::actions::*`) explicitly remain UI-side, deferred to Phase 6 alongside the relocation. |
| Contacts / GAL refresh writes | `handlers/contacts.rs`, GAL refresh in `handlers/provider.rs` | GAL refresh: Phase 5 - LANDED. UI fires `gal.kick` on `Message::SyncTick`; Service handler iterates accounts under a global Mutex (NOTIFY_CAP=4 hazard); 24h cache check inside `refresh_gal_for_account` self-throttles. Contacts CRUD writes still in `handlers/contacts.rs`: Phase 6. |
...
**Phase 4 status (as landed).** Of the Phase 4 entries above:
- ✓ JMAP push WebSocket receiver moved into the Service. `crates/service/src/push.rs::PushRuntime` mirrors `SyncRuntime`'s shape - per-account map, panic supervisor, lifecycle hooks. Bridge body owns the `JmapPushManager` and runs `manager.stop_push().await` on its exit path so the WebSocket connection-loop terminates cleanly (the only correct teardown signal; dropping the manager alone leaves the connection-loop's `watch::Receiver` waiting forever).
- ✓ `service-api` push surface: `Notification::PushEvent { account_id, service_generation }` (Coalesce class, per-account latest-wins). No new request methods - the start/cancel piggyback on the existing `sync.start_account` / `sync.cancel_account` IPC, fire-and-forget so push setup latency doesn't violate the 5s no-network IPC contract.
- ✓ OAuth refresh runs in-Service. `JmapClient::ensure_valid_token` is called before `start_push`, and an auth resolver (`Arc<dyn Fn() -> BoxFuture<'static, Result<String, String>>>`) is threaded into `push_connection_loop` so reconnects re-resolve the bearer token. The "temporary `oauth.refresh_request` IPC" from the original roadmap entry was a planning-doc error; refresh is purely DB+HTTPS, both Service-internal.
- ✓ Drain consolidation (Phase 3 carry-forward fix): PushRuntime → SyncRuntime → sentinel write, all in one orchestrated helper.
- ✓ Post-ready push startup: `dispatch.rs::spawn_post_ready_push_startup` waits for `boot.ready`, iterates JMAP accounts, `tokio::spawn`s a per-account start. No new `BootPhase` variant - readiness must not depend on TLS+HTTPS+OAuth-refresh latency.
- ✓ UI teardown: `core/src/jmap_push.rs` deleted; `Message::JmapPushKick` deleted; `JmapPushReceiver` / `create_jmap_push_channel` / `jmap_push_subscription` deleted; `start_jmap_push()` call site at `handlers/core.rs:1014` deleted; `App.jmap_push_tx` / `App.jmap_push_receiver` fields deleted. UI's `update.rs` gains a `Notification::PushEvent` arm that stamps `StatusBar::last_push_at[account_id]` for status-bar visibility.

Crash continuity (`jmap_push_state` resume) is *inherited* unchanged - today's `start_push` already loads saved state at `crates/jmap/src/push.rs:147` and sends it in `WebSocketPushEnable` at `:337`. Phase 8 hardens this with crash-aware fresh-start logic. The original Phase 4 plan draft framed this as "ignored," which was factually wrong; the revision-pass corrected it.

Two Phase 4 known-gaps deferred to Phase 8: (a) re-auth doesn't re-arm dead push entries (UI re-auth updates the row in place; doesn't trigger `PushRuntime::start_account`), workaround is Service restart; (b) integration test cohort needs a fake-JMAP-server fixture or `test_dummy` constructors on the writer-state types - both infrastructure work that's already on Phase 8's plate alongside the flaky-test root-cause. The PushEvent wire/class guarantees are covered by the catalog tests in `crates/service-api/src/notification.rs:469-585`.

**Phase 4 post-implementation review-pass fixes.** A `review arch,bugs --oneshot` against the landed Phase 4 commits surfaced nine findings, three rated High by both bug reviewers. Follow-up commits closed every finding before phase-4-plan.md retirement:

- ✓ PushRuntime shutdown guard (`closed: AtomicBool`) - blocks `start_account` from racing past `shutdown()` via the three back doors (post-ready iteration's untracked spawns, `handle_start_account` piggyback, `handle_cancel_account` piggyback). Fix bundled with the lock-held-across-network restructure (`start_account`'s slow path now runs outside the accounts mutex; second mutex acquisition for the insert re-checks both the shutdown guard and the duplicate-entry guard).
- ✓ `JmapPushManager::stop_push().await` actually awaits the WebSocket loop. Pre-fix the connection-loop's `JoinHandle` was discarded; `stop_push` was a sync `watch::send` followed by an async-marker no-op return. Now the manager owns the handle (`Mutex<Option<JoinHandle>>`); `stop_push` sets the watch *and* awaits the loop's termination. Bridge exit path is genuinely "WebSocket closed" rather than "watch flipped."
- ✓ Cold-startup OAuth ordering. `JmapClient::from_account` now refreshes the bearer token *before* the JMAP `connect()` (extracted DB-side helper `refresh_token_in_db_if_expired`). `ensure_valid_token`'s gate widened to `"oauth2" | "bearer"` to match `from_account`'s existing branch (closes the silent-bypass for `bearer`-method accounts).
- ✓ Account-delete cancel ordering. `handle_cancel_account` now `await`s `PushRuntime::cancel_account` before returning the ack. UI's `cancel_and_await` contract is now strict: by the time it returns, both sync runners and push bridges for the account have torn down. The 5s IPC budget holds because the connection loop's TLS connect await and `auth_resolver().await` are now in `tokio::select!` against `shutdown_rx` (so `stop_push` is bounded latency).
- ✓ Search-writer `JoinHandle` awaited at drain. Pre-existing Phase 3 bug: the consolidated drain doc-comment claimed "step 5: await search-writer JoinHandle" but the implementation discarded the handle. Worked today only via the undocumented `flush_now`-on-every-sync-exit-path invariant. `search_writer::spawn` now returns the `JoinHandle`; `BootSharedState::install_search_writer_handle` slot mirrors the sync_runtime / push_runtime triple; consolidated drain awaits it after dropping `Arc<SyncRuntime>`.
- ✓ `production_notification_catalog` test extended with `PushEvent` so the cross-respawn round-trip test catches the new variant.
- ✓ Dead-code `StatusBar::last_push_at` accessor + the aspirational verification claim replaced with an actual rate-limited "New mail in &lt;email&gt;" confirmation render. Confirmation fires at most once per `PUSH_CONFIRMATION_INTERVAL = 60s` per account so a heavy import doesn't spam the status bar.

The "PushRuntime integration test cohort" Phase 8 carry-forward stays in place; the review pass identified that the start-after-shutdown invariant is unit-testable today against `PushRuntime` alone (no fake-JMAP fixture), which is true and will be picked up alongside the rest of the cohort when the fake-JMAP infrastructure lands.

**Phase 5 status (as landed).** Of the Phase 5 entries above:

- ✓ Cycle break prerequisite. New `crates/action-types/` crate holds `ActionContext`, `FlightGuard`, `ActionError`, `ActionOutcome`, `RemoteFailureKind`, `MutationLog`. Both `service::actions` and `cal::actions` depend on it directly. The three `rtsk -> service` edges that kept the cycle alive are gone: the `core::actions` shim (deleted), the `pub use service::sync_dispatch` re-export from `core::lib.rs` (deleted), and the dead `core::chat::mark_chat_read_remote` (deleted; the live equivalent has been in `service::actions::worker::mark_chat_read_remote` since Phase 2's relocation). `cargo metadata` is cycle-clean.
- ✓ IMAP cancellation depth. The `let _cancellation_token = cancellation_token;` discard markers in `imap_initial_sync` / `imap_delta_sync` are gone. `&CancellationToken` now threads through the per-folder loops, per-batch persist points, and helper paths (`batch_delta_check`, `imap_delta_janitor::run_deletion_detection`, `client::sync::delta_check_folders`). Point-checks between RPCs, not `tokio::select!` - IMAP is a stateful session: dropping a future mid-FETCH leaves unread response data on the wire and breaks the next command. Worst-case observed cancel latency is bounded by `IMAP_FETCH_TIMEOUT` against a real provider; sub-2s against the in-process IMAP stub.
- ✓ Calendar sync relocated Service-side. New `crates/service/src/calendar.rs::CalendarRuntime` mirrors `SyncRuntime`'s lifecycle surface (per-account map, panic supervisor, `start_account -> Result<CalendarStartAck, String>`, `cancel_account -> CalendarCancelAck`, `shutdown`). Per-runtime semaphore caps concurrent runners (DEFAULT_CONCURRENCY_CAP = 4) so a Service respawn that resets in-memory `last_completed` doesn't trigger an N-account thundering herd. `closed: AtomicBool` shutdown guard mirrors `PushRuntimeInner`'s. No marker-file lifecycle - calendar sync is idempotent against CalDAV CTags / Exchange ETags.
- ✓ `WriteDbState` flip at the Service boundary. `cal::sync::calendar_sync_account_impl` and the `calendar_sync_account` wrapper now take `&service_state::WriteDbState`. Body derives a `ReadDbState` view internally so deeper helpers (still on `&ReadDbState::with_conn` for writes) compile unchanged - that escape moves into a Phase 6 cleanup alongside `cal::actions`'s relocation. The Service-facing surface is honest about needing write capability.
- ✓ `service-api` calendar wire types. `CalendarRunId` (UUIDv7), `CalendarStartAccountSyncParams`, `CalendarStartAck { account_id, run_id, already_in_flight }`, `CalendarCancelAccountSyncParams`, `CalendarCancelAck { account_id, run_id, was_in_flight }`, `CalendarSyncResult { Completed | Cancelled | Failed }`. Two notifications: `CalendarRunCompleted { account_id, run_id, result, mutated, service_generation }` (MustDeliver, ServiceClient-consumed by per-run_id awaiters) + `CalendarChanged { account_id, service_generation }` (Coalesce, `CoalesceKey::CalendarChanged(account_id)`, UI-dispatched for view reload). Two `ClientNotification` variants: `CalendarKick`, `GalKick` (both Drop class, mirror `PendingOpsKick`). Two `RequestParams` variants: `CalendarStartAccountSync`, `CalendarCancelAccountSync` (both 5s timeout). `SyncCancelAck` extended with `calendar_run_id: Option<CalendarRunId>` so account-deletion piggyback can correlate calendar terminal completion.
- ✓ Service-side handlers. `handlers/calendar.rs::handle_start_account_sync` / `handle_cancel_account_sync` forward to `CalendarRuntime`. `handle_calendar_kick` enumerates accounts past 1h staleness and starts each (start_account is idempotent; per-runtime semaphore caps concurrency). `handlers/gal.rs::handle_gal_kick` iterates all accounts under a global Tokio Mutex (the `NOTIFY_CAP=4` concurrent dispatcher would otherwise duplicate provider calls for stale accounts), 60s per-account timeout. `handlers/sync.rs::handle_cancel_account` extended to also call `CalendarRuntime::cancel_account` and stamp `ack.calendar_run_id` for the deletion-piggyback path.
- ✓ Drain consolidation. Order: `PushRuntime` → `CalendarRuntime` → `SyncRuntime` → search-writer → sentinel. The Calendar-before-Sync ordering is **reserved**, not load-bearing today (the action worker is alive throughout the drain so calendar can drain before or after sync without affecting action-worker availability). The order is fixed so a future change wiring calendar-cancel cleanup to dispatch action plans (RSVP send) is a one-liner instead of a drain reshuffle. Notification drain is bounded at 5s aggregate; past it remaining notification tasks are aborted and the count is logged (handlers wrapping work in `spawn_blocking` see the *outer* future aborted; the blocking closure runs to completion - GAL writes are bounded and idempotent so this is acceptable).
- ✓ UI: `Message::SyncTick` collapses to four kicks. `sync_all_accounts` (request fan-out) + `process_pending_ops` (`pending_ops.kick`) + `kick_gal_refresh` (`gal.kick`, NEW) + `kick_calendar_sync` (`calendar.kick`, NEW). `Message::GalRefreshTick` and the dedicated 1-hour subscription deleted; UI-side `sync_calendars` and `refresh_gal_caches` deleted. Two cadences = two failure modes; one cadence (5-min `SyncTick`) + Service-side staleness gates (calendar 1h per-account, GAL 24h cache check) is the simpler shape and survives the Phase 9 tray-resident move unchanged.
- ✓ UI: `Notification::CalendarChanged` debounced reload. 250ms trailing-edge debouncer feeds a single `reload_calendar_events()` call so an N-account kick batch produces one reload, not N. `Notification::CalendarRunCompleted` is consumed inside `ServiceClient`'s reader task by per-run_id awaiters (mirrors `SyncCompleted`); never reaches the UI dispatcher.
- ✓ Account-deletion path: `cancel_and_await` awaits both sync and calendar terminal completions before issuing the DB DELETE. The `ack.calendar_run_id` carries the calendar run id; `tokio::join!` lets the two cancels overlap; the DELETE is safe against calendar tables CASCADE from `accounts`.
- ✓ Catalog tests: `production_notification_catalog` gained `CalendarRunCompleted` and `CalendarChanged` so the cross-respawn round-trip test catches the new variants.

Phase 5 known-gaps:

- **Calendar event mutations (`cal::actions::*`) still run UI-side.** `crates/app/src/handlers/calendar.rs` still calls `cal::actions::*` directly with a UI-side `ActionContext`. Phase 5 relocates only the *periodic provider sync/cache refresh*; event-mutation relocation is Phase 6 territory. The `cal::actions` write-surface escape (writing through `&ReadDbState::with_conn`) is the same shape Phase 4 cleaned up for sync; Phase 6 retires it alongside the relocation.
- **`mutated` flag is coarse.** `CalendarRuntime` emits `CalendarChanged` whenever `result == Completed` or `result == Cancelled`, conservatively treating any cancellation past the entry checkpoint as "may have committed a batch." Pure failures emit only `CalendarRunCompleted`. Tightening the cancellation-without-mutation case to skip the reload requires threading a `mutated: &AtomicBool` flag through `cal::sync` end-to-end; that's deferred to the Phase 6 cleanup pass which already needs to refactor those helpers.
- **Phase 9 tray-resident scheduler ownership.** Today the iced `SyncTick` subscription drives all four kicks. Once the app gains a tray-resident mode, refresh stops working when the UI window is closed. The Service knows enough to drive its own cadence and should take it over at that point. Marker `// TODO(phase-9): revisit when tray-resident lands` lives in `app/src/subscription.rs` near the SyncTick subscription. Phase 5 deliberately keeps the cadence UI-side to avoid building a Service-side scheduler that has to be re-designed once tray-resident ships.
- **Integration test cohort partly Phase 8.** Lifecycle-only unit tests landed in this phase (`CalendarRuntime::cancel_account` returns None for missing entries, `shutdown` is safe on empty runtime, `start_account` returns Err after shutdown). Real runner behavior against a stub calendar provider needs either a fake-CalDAV fixture or `test_dummy` constructors on the writer-state types - same caveat as Phase 4's PushRuntime cohort. Carries forward to Phase 8 alongside the flaky-test root-cause work.
...
**Phase 5 (calendar sync state).** No recovery pass needed. CalendarRuntime has no marker-file lifecycle: calendar sync is idempotent against CalDAV CTags / Exchange ETags, so a Service crash mid-calendar-sync resumes naturally on the next kick (the provider re-reports whatever changed and the next batch reconciles). The kick handler's in-memory `last_completed` map is lost on respawn; the first kick after restart re-syncs every account but the per-runtime semaphore caps the concurrency, and idempotency on the wire makes the cost bounded.
