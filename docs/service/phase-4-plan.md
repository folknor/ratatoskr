...
**2026-05-05 - post-implementation review pass.** After Phase 4 tasks 1-10 landed (commits `5886c7c3..4f3ff9e4`), the implementation was reviewed by `review arch,bugs --oneshot` (claude + codex sessions for each). Strong convergence on nine findings; three rated High by both bug reviewers; the deferred test cohort was identified as overcautious for at least one finding (the `start_account`-after-`shutdown` invariant is unit-testable today against `PushRuntime` alone, no fake-JMAP fixture needed). Follow-up commits land before phase-4-plan.md retirement. Findings + resolutions:

- **P1 - `PushRuntime::start_account` has no shutdown guard.** All four reviewers identified the same race: `start_account` accepts new entries after `shutdown()` has begun. Three back doors: (a) post-ready iteration's per-account `tokio::spawn` (untracked children continue running after the parent task aborts), (b) `handle_start_account` piggyback (not in `handlers_in_flight`, so `drain_in_flight` doesn't await it), (c) `handle_cancel_account` piggyback (same). A late-spawned bridge holds an `Arc<NotificationSender>` clone of `out_tx`, hangs `writer_handle.await` indefinitely, and can call `SyncRuntime::start_account` after sync drained - re-opening the Phase 3 sentinel race the consolidation was supposed to close. **Resolution:** add `closed: AtomicBool` to `PushRuntimeInner`; flip in `shutdown()` *before* taking the snapshot; gate `start_account` on it (return `Err` after shutdown observed). Belt-and-suspenders symmetric guard on `SyncRuntime::start_account` for any future bypass.

- **P1 - `manager.stop_push().await` doesn't actually await the WebSocket loop.** `JmapPushManager::stop_push` is a sync `let _ = self.shutdown_tx.send(true)` followed by an immediate return - the `.await` is a no-op. The connection-loop's `JoinHandle` was discarded at construction (`crates/jmap/src/push.rs:187`). After `PushRuntime::shutdown` returns, the connection loop can survive past the sentinel write and emit late `save_last_push_state` writes that corrupt the resume state Phase 8 is supposed to harden. **Resolution:** capture the connection-loop's `JoinHandle` inside `JmapPushManager`; `stop_push` becomes genuinely awaiting (sets the watch, then awaits the handle). Bridge exit path is then truly "WebSocket closed."

- **P1 - Cold-startup OAuth ordering inverted; refresh skipped for `"bearer"` auth method.** Two related issues. (a) `JmapClient::from_account` connects to the JMAP server *before* `ensure_valid_token` runs in `PushRuntime::start_account` - a Service that was down through token expiry fails the initial connect with the stale token and exits via `MAX_CONSECUTIVE_FAILURES`. (b) `ensure_valid_token` checks `if self.auth_method != "oauth2"` and short-circuits, but `from_account` accepts both `"oauth2"` and `"bearer"` as OAuth-like (`client.rs:269`). A `"bearer"`-method account silently bypasses refresh. **Resolution:** call `ensure_valid_token` *inside* `JmapClient::from_account` *before* the JMAP `connect()` call, and widen the `auth_method` check to match `from_account`'s list (`"oauth2" | "bearer"`).

- **P1 - Account-delete cancel ordering races push.** UI's `cancel_and_await` only awaits sync; Service's `handle_cancel_account` detaches push-cancel as fire-and-forget (Phase 4 task 6). UI proceeds to DB delete while the push bridge is still alive and could fire one more `start_sync` against the disappearing account. **Resolution:** make `handle_cancel_account` `await` `PushRuntime::cancel_account` before returning the ack. Cancel is fast (cooperative token cancel + supervisor `JoinHandle` await + `manager.stop_push().await` which now actually awaits per the fix above) - no TLS/HTTPS in the cancel path, so the 5s IPC budget is comfortable. Symmetric IPC contract: cancel awaits both subsystems; start awaits only sync.

- **P2 - `PushRuntime::start_account` holds the `accounts` mutex across `ensure_valid_token` + `start_push` + supervisor spawn.** SyncRuntime's mutex usage is sub-ms (atomic marker write); PushRuntime's is N × (refresh + connect). One wedged provider start blocks all other push starts/cancels/shutdown, and widens the start-after-shutdown race surface in #1. **Resolution:** restructure `start_account` so the slow path runs outside the lock - acquire lock for the contains_key check, drop, do the slow work, re-acquire to insert (and bail if a concurrent inserter won the race; cancel-the-loser is handled by `JmapPushManager::stop_push`).

- **P3 - Search-writer `JoinHandle` is not awaited at drain.** Pre-existing Phase 3 issue. The Phase 3 search_writer task is `tokio::spawn(...)` with the JoinHandle discarded; the consolidated drain's doc-comment claims "step 5: await search-writer JoinHandle" but the implementation does not. Works today only because every `run_sync` exit path calls `flush_now().await` which round-trips through an oneshot ack - so by `SyncRuntime::shutdown` return, the writer is queued-empty. Undocumented invariant; one stray future change to a sync exit path that skips flush re-opens a sentinel-before-flush race. **Resolution:** `spawn_search_writer` returns the `JoinHandle`; `SyncRuntime::shutdown` awaits it after dropping its `SearchWriteHandle`. Phase 3 sidebar fix bundled with the other lifecycle fixes.

- **P3 - Bare `auth_resolver().await` in `push_connection_loop`.** Not in a `select!` against `shutdown_rx`. A hanging refresh (network partition mid-`refresh_oauth_token`, DB lock contention) parks the loop indefinitely; setting the shutdown watch from the bridge exit path won't wake it until the await unblocks. **Resolution:** wrap the resolver call in `tokio::select!` with `shutdown_rx.changed()` so a shutdown observation breaks out of the resolver wait and lets the loop exit promptly.

- **P3 - `production_notification_catalog` test missing `PushEvent` arm.** `crates/app/src/service_client.rs:2413-2437`'s catalog enumerates state-changing notification variants for the cross-respawn round-trip test; Phase 4 added `PushEvent` and didn't extend the catalog. The wire-level catalog tests in `service-api/src/notification.rs:469-585` cover serde + class + service_generation, but the UI-side cross-respawn drop-on-stale-generation flip is not exercised. **Resolution:** add a `PushEvent` case to the production catalog test.

- **P3 - `StatusBar::last_push_at` accessor is dead code.** `record_push_event` is wired (Phase 4 task 8), but `last_push_at(account_id)` is `#[allow(dead_code)]` and no view consumes it. The plan's verification line "Status bar shows 'new mail arrived' within the debounce window" is therefore aspirational. **Resolution:** wire a transient indicator into the status-bar view layer that reads `last_push_at` and decays after a short window. If the indicator render is more involved than a one-line tweak, fall back to: remove the `#[allow(dead_code)]` accessor and its TODO entirely, and delete the verification claim from the plan; honest-no-feature beats dishonest-feature-flag.
