...
13. **`service`: `action.send` handler with bytes-ownership transfer before ack.** Compose-send relocates. New `SendWireRequest` with `SendAttachmentSource::StagingFile { relative_path, content_hash }` / `PackStore { content_hash }` (per scope item 5). UI's `crates/app/src/handlers/pop_out/compose_send.rs::dispatch_send` writes attachment bytes to `<app_data>/staging/<send_id>/<index>.bin`, then issues `client.send(SendWireRequest)`. Service's `crates/service/src/handlers/action.rs::handle_send`: (a) validates each `relative_path` against `..`/absolute/symlink traversal; (b) re-reads bytes, verifies hash, atomically renames/copies into Service-owned `<app_data>/send_vault/<job_id>/<index>.bin`; (c) for `PackStore` refs, bumps refcount; (d) journals the send into `action_jobs` with `kind = 'send'`, payload contains vault paths (not staging paths); (e) returns `SendAck { send_id, journaled: true }`. Worker reads from the vault, handles SMTP submit, emits progress + final `SendCompleted`, unlinks vault directory on terminal status. Boot recovery unlinks any vault directory whose job is missing/terminal. Staging directory is the UI's responsibility.
