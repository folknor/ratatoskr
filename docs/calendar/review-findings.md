...
- **High** - Inverted error signaling: malformed rule (unknown FREQ, empty FREQ field) returns `vec![event.clone()]` so the event renders once; well-formed-but-zero-instance rule (UNTIL in past, BYDAY excludes everything) returns `vec![]` so the event vanishes. No error channel back to the caller and no log on malformed-rule.
...
- **Medium** - CDATA decode failures (`Event::CData(e)` where `e.decode()` errors) silently drop the entire payload; event missing from sync with no diagnostic. Add WARN on decode failure.
