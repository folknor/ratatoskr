...
## Sequencing relative to `crate-survey.md`

When the calendar code is picked back up, the deferred work catalog in
`docs/calendar/crate-survey.md` is the strategic plan; this document is
the open punch list from the 4 review rounds. They overlap in places.
...
four are independent and can be picked up opportunistically. When
returning, start with the catalog directly.
