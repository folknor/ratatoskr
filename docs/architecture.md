...
**Per-message membership store with derived thread aggregate** - `message_keywords` (`crates/db/src/db/schema/02_mail.sql`) is the first per-message membership table: keys `(account_id, message_id, keyword, label_id)`, FK-cascaded by message. The thread-level `kw:%` rows in `thread_labels` are recomputed from the union of `message_keywords` rows for the thread's messages by `recompute_thread_keyword_labels` (`crates/provider-sync/src/keyword_membership.rs`). This is what makes IMAP/JMAP keyword removal observable - the previous design had no per-message store, so a server removing a keyword could not subtract from the thread aggregate. The pattern is the prototype for future per-message folder/category tables: any time the thread aggregate has to track *removal* across partial provider deltas, the per-message store + recompute is the shape. The cross-client folder/label move TODO points at this as the fix shape for Graph and JMAP.

**Partial-delta membership merge vs full-thread replace** - Thread folder and raw-label writes follow one of two provider-local paths depending on whether the provider gives full-thread or partial-thread snapshots. Gmail storage and the IMAP thread-store pass (`crates/provider-sync/src/imap/thread_store.rs`) call private replace wrappers after collecting the full folder/label union, so destructive replacement is safe. Graph and JMAP sync modules call private merge wrappers, which insert new rows but never remove because their delta pages contain only changed messages. The shared `thread_membership` module owns defensive provider-side filtering; `db::queries_extra::thread_persistence` exposes only raw row insert/delete primitives. The trade-off: merge preserves sibling-message memberships that the delta does not mention, but cannot detect that a folder is gone after a cross-client move because the delta only describes what the message *is* in, not what it is no longer in. Same-client moves are unaffected - the action service updates `thread_folders` locally before dispatching. Cross-client move reconciliation is tracked in `TODO.md`; the long-term fix is the per-message membership store pattern (above).
