...
**Partial-delta membership merge vs full-thread replace** - Thread folder and raw-label writes follow one of two helper families depending on whether the provider gives full-thread or partial-thread snapshots. `replace_thread_folders` / `replace_thread_labels` delete the thread's existing rows for their junction and insert the input set. Used by Gmail and IMAP via `crates/sync/src/pipeline.rs::store_thread_groups_to_db`, which has already collected the union of folder IDs and raw label IDs across every message in the thread, so the destructive replace is safe. `merge_thread_folders` / `merge_thread_labels` insert new rows but never remove; used by Graph and JMAP, which receive partial delta pages containing only changed messages. The trade-off: merge preserves sibling-message memberships that the delta does not mention, but cannot detect that a folder is gone after a cross-client move because the delta only describes what the message *is* in, not what it is no longer in. Same-client moves are unaffected - the action service updates `thread_folders` locally before dispatching. Cross-client move reconciliation is tracked in `TODO.md`; the long-term fix is the per-message membership store pattern (above).
