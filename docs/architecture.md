...
**Pre-create labels rows for any label_id that lands in `thread_labels`.** Three call sites enforce this so the LABELS sidebar enumeration and the `add_label` action gate never see orphan IDs: Graph upserts `cat:{name}` rows alongside the `thread_labels` write in `crates/provider-sync/src/graph/sync/persistence.rs::upsert_graph_label_rows`; IMAP upserts `kw:{keyword}` rows in `replace_message_keywords` (`crates/provider-sync/src/imap/sync_pipeline.rs`); JMAP upserts `kw:{keyword}` rows in `sync_keyword_labels` (`crates/provider-sync/src/jmap/sync/storage.rs`). Synthesised tag-kind rows for Graph importance (`importance:high`, `importance:low`) are upserted in the same Graph persistence path. Master-list pulls (`graph_label_sync` for Exchange categories) still run on a slower cadence and provide colour metadata, but the message-persist upsert is the immediate guarantee.
...
**Per-message membership store with derived thread aggregate** - `message_keywords` (`crates/db/src/db/schema/02_mail.sql`) is the first per-message membership table: keys `(account_id, message_id, keyword, label_id)`, FK-cascaded by message. The thread-level `kw:%` rows in `thread_labels` are not written directly; they are recomputed from the union of `message_keywords` rows for the thread's messages by `recompute_thread_keyword_labels` (`crates/provider-sync/src/imap/sync_pipeline.rs`). This is what makes IMAP keyword removal observable - the previous design had no per-message store, so a server removing a keyword could not subtract from the thread aggregate. The pattern is the prototype for future per-message folder/category tables: any time the thread aggregate has to track *removal* across partial provider deltas, the per-message store + recompute is the shape. The cross-client folder/label move TODO points at this as the fix shape for Graph and JMAP.

**Partial-delta thread label merge vs full-thread replace** - Thread-label writes follow one of two helpers depending on whether the provider gives full-thread or partial-thread snapshots. `replace_thread_labels` (`crates/db/src/db/queries_extra/thread_persistence.rs`) deletes the thread's existing rows and inserts the input set. Used by Gmail and IMAP via `crates/sync/src/pipeline.rs::store_thread_groups_to_db`, which has already collected the union of `label_ids` across every message in the thread, so the destructive replace is safe. `merge_thread_labels` (same file) inserts new rows but never removes; used by Graph (`crates/provider-sync/src/graph/sync/persistence.rs`) and JMAP (`crates/provider-sync/src/jmap/sync/storage.rs`), which receive partial delta pages containing only changed messages. The trade-off: merge preserves sibling-message memberships that the delta does not mention (the bug the helper was added to fix), but cannot detect that a folder is gone after a cross-client move because the delta only describes what the message *is* in, not what it is no longer in. Same-client moves are unaffected - the action service updates `thread_labels` locally before dispatching. Cross-client move reconciliation is tracked in `TODO.md`; the long-term fix is the per-message membership store pattern (above).
