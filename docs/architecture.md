...
The `ExtractRuntime` lifecycle mirrors `CalendarRuntime`: a `CancellationToken` + stored worker `JoinHandle` + `Arc<Mutex<HashSet<content_hash>>>` enqueue dedupe. The drain order is `Push -> Calendar -> Sync -> Extract -> Rebuild (single-task) -> search-writer -> sentinel`. The `search_write` slot on `BootSharedState` is single-use - `take_search_write` is called from inside `spawn_post_ready_extract_startup` on success and defensively from `run_shutdown_drain` before the search-writer await, so no `SearchWriteHandle` clone leaks past the writer-task EOF observation (the bug that bricked `boot_ready_blocks_until_sequence_completes` during the 7-4d revival).
...
- **Strict transitive `service-state` lockdown.** `crates/sync/Cargo.toml` and the four provider crates (`crates/{gmail,jmap,graph,imap}/Cargo.toml`) still depend on `service-state` directly. The Phase 6b direct-dep check + Phase 6c-11 `app -> cal` transitive check together close every UI-reachable path that the workspace shape allowed at the time, and `WriteDbState` is constructed only inside `service-state` and `service`, so app code cannot accidentally reach a writer. Closing the remaining edges would require `SyncProviderCtx` out of `common`, `sync` split into pure-logic + persistence halves, and the per-provider sync impls relocated to a sibling crate. That is its own multi-phase project and is not currently scheduled. The lockdown integration test (`crates/service-state/tests/lockdown.rs`) is the practical contract; the strict transitive variant is documented here so the gap is visible.
- **Compose-send network harness coverage.** `compose_send_50mb_attachment`, `send_wire_attachment_validation`, and `send_wire_oversize_payload_handler_path` are listed in harness M4's remaining scope (`docs/harness/roadmap.md`). They are blocked on the saehrimnir mock SMTP path, not on ratatoskr-side work. The action pipeline itself is fully in tree.
