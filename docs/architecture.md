...
Schema-version mismatch (when `INDEX_SCHEMA_VERSION` bumps) takes a Wipe-rebuild path: a `.version` sentinel under `<search_index>/.version` is read inside `BootPhase::OpeningSearchIndex` *before* the writer spawn; on mismatch a flag is set on `BootSharedState` and the post-ready dispatcher fires `handle_rebuild` with `RebuildPolicy::Wipe`, polls until completion, then writes the current `.version`. Sentinel-write ordering is preserved: a mid-rebuild crash leaves the OLD `.version` on disk and the next boot re-fires. The user-triggered palette command "Rebuild Search Index" (`CommandId::AppRebuildSearchIndex`) routes through the same `handle_rebuild` with `RebuildPolicy::Wipe`. v1 ships search-briefly-unavailable during rebuild; the search-stays-live PreserveExisting dual-index path (legacy + next directories with atomic swap) is a Phase 8 carry-forward.
