...
2. **Unread count granularity in unified view**: Should "Inbox 12" in the unified view be expandable to show per-account breakdown (Foo: 7, Gmail: 3, Bar: 2), or is the total sufficient? A breakdown helps triage ("the 7 are all work") but adds visual complexity. Current bias: totals only in the default layout. If per-account attribution proves necessary, it should surface as a hover tooltip or secondary affordance rather than inline breakdown — a sidebar full of per-account numbers would damage scanability.
...
4. ~~**Pinned labels/folders**~~ **Resolved: not in V1.** The unified view's strength is that it is intentionally sparse — universal folders plus smart folders, nothing else. Allowing pinned per-account labels would collapse this clean information architecture. Power users who need a specific label always visible can create a smart folder with `account:FooCorp label:Clients` — that achieves the same result without polluting the unified navigation model. Revisit only if there is strong post-launch evidence that smart folders don't satisfy the use case.
...
6. **Scope persistence**: Should the active scope be persisted so that relaunching the app restores the user's account context? Currently scope lives in the iced app model as in-memory state and resets on restart. Options: persist to the SQLite settings table, or treat "All Accounts" as a reasonable default on launch. Caution: if scope is persisted and the app restores into a narrow account scope, the user needs very clear visual context (account name/color prominent in the sidebar header) to avoid a "hidden mode trap" where they don't realize they're scoped and wonder why emails are missing.
