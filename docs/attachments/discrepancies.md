...
## Test gaps from the Phase 7/8/9 review batch (now covered)

- **IMAP folder-batch "one LOGIN + one SELECT per folder" assertion.** Covered by `imap-folder-batch-session-reuse.lua` against `imap-attach-multi.toml`. Saehrimnir now tags every `RequestEntry` with a process-wide `connection_id` (dense-indexed under `?stable=true`), so the harness can group IMAP commands by session. The script seeds an IMAP account with three attachments in INBOX, syncs, waits for prefetch.completed, then groups body fetches by `connection_id`: the session with the most body fetches is the prefetch one, and the assertion is that it issued exactly 1 LOGIN and 1 SELECT regardless of how many attachments it batched. A regression to one-session-per-attachment would split the three fetches across three connection_ids and the "max body fetches per session >= 3" assertion would fail.
- **Cross-account shared-blob deletion harness.** Covered by `imap-account-delete-shared-blob.lua` against `multi-account-attach.toml`. Script seeds two IMAP-routed accounts with one shared attachment (BLAKE3-dedupes to a single `attachment_blobs` row) plus one unshared attachment, deletes the first account, and asserts the shared blob survives (other account still references) while the unshared blob is tombstoned (Phase 4's `AccountDeletionStep::AttachmentCache`). IMAP rather than JMAP because saehrimnir's JMAP session response pins `primaryAccounts` to the fixture's `primary = true` account regardless of bearer, so two ratatoskr accounts would both sync the same JMAP scope. IMAP LOGIN routes correctly per credential and the behavior under test is protocol-agnostic.
