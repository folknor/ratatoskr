...
stdio. Tests for the Service's lifecycle - boot handshake, shutdown
drain, drop behaviour, parent-death, signal handling, crash recovery,
schema-version respawn, JSON-RPC framing defense - currently live as
`#[tokio::test]` functions in `crates/app/tests/service_subprocess.rs`.
Each test spawns the `app --service` binary, drives the protocol over
its stdio, and asserts on observed behaviour with internal
...
**Failure destroys the diagnostic state.** `DataDirGuard::Drop` cleans
up the test's app-data directory unconditionally - including on test
failure. SQLite WAL state, the lockfile, the key file, the
`clean_shutdown` sentinel: all gone the moment the test fails. There is
no `/proc` snapshot, no preserved frame log, no preserved Service
stderr. A failure today produces "test timed out at line N" with
nothing to attach to a bug report. Re-running the test in a debugger is
the only path forward, and the bug may not reproduce twice.
...
can't carry them. Phase 8's planning notes are explicit: "building T1
...
A test runtime that makes Service-lifecycle tests **deterministic by
construction**: every wait races a predicate against the child process
exiting, plus a named backstop. First transition to fire wins; the
runtime records which one fired in the test trace. Backstops remain
wall-clock - the runtime cannot escape physical time entirely - but
they are explicit, named, generous, and only fire when a determinism
bug elsewhere leaves the runtime with nothing else to wait on. A
backstop firing is a test-design bug, not a flake.

The runtime is **generic enough to drive any JSON-RPC subprocess.**
Ratatoskr's Service is the first user; nothing in the runtime's shape
is Service-specific beyond the `ServiceClient` userdata bindings, which
are themselves a wrapper around an existing public Rust API. A second
project that needs the same shape can reuse the runtime and write its
own bindings.
...
out" and nothing else. The data dir is gone. The `/proc` state is
gone. The frame log was never captured. The Service stderr was
race-mingled with the test runner's stdout. The only path forward is
to re-run with extra `dbg!` calls and hope the failure reproduces.
For drain-ordering bugs (the most common Service-correctness class),
hangs reproduce once in twenty runs.
...
- **Not a cargo-test wrapper.** Cargo's single-test runner (`brokkr
  test`) stays for unit tests and tests that don't need the
  real-subprocess shape. The harness is specifically for tests where
  the subprocess lifecycle is the thing under test.
- **Not a migration target for the existing tokio-tests.** New tests
  start in the harness; existing `#[tokio::test]` functions in
  `service_subprocess.rs` migrate only if their authors choose to.
  The two `#[ignore]`'d ones land here as the wedge.
