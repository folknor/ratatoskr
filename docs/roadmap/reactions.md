...
2. [x] **Phase 2 (Gmail read)**: `extract_reaction_emoji()` in `gmail/src/parse.rs` parses `text/vnd.google.email-reaction+json` MIME parts during sync. resolving the target via `In-Reply-To` and populating `message_reactions` with `source = 'gmail_mime'` now lives in the bifrost consumer's Gmail write arm (`insert_gmail_reaction()` in `crates/service/src/bifrost/consumer/write.rs`), relocated there from the deleted `provider-sync` `gmail/sync/storage.rs::insert_reactions` at the B3a-cut-gmail bifrost cutover. Migration v37 in `db` crate.
3. [x] **Phase 3 (Exchange read)**: relocated to the bifrost cutover at B15c. `crates/graph/` (the legacy client, including `parse.rs::extract_reaction_properties` and `sync/persistence.rs::insert_exchange_reactions` / `refresh_reactions_for_recent_messages`) is deleted; bifrost-graph reads `OwnerReactionType` and `ReactionsCount` (GUID `{41F28F13-...}`) via a `$batch`-chunked `SyncEngine::message_reactions` call returning a three-lane `BatchOutcome`. The pure classifier (`classify_reaction_updates` in `crates/service/src/bifrost/aux/graph.rs`) maps only the succeeded lane to row ops - a failed/uncertain batch item is never treated as "no reaction", which would otherwise wipe cached reactions on a transient Graph error - and `run_graph_auxiliary_sync` in the same file still refreshes every 5th `graph_sync_cycle` tick to catch reaction changes missed by delta queries. Two pre-existing bugs are ported faithfully, not fixed: a cleared reaction's `__count__` row is never deleted, and a changed reaction/count value inserts a second row instead of updating in place (schema conflict-target mismatch on `message_reactions`).
