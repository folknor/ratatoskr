...
> **Superseded by [Labels Unification](../labels-unification/redesign.md).**
> Post-split, Exchange categories, IMAP keywords, and JMAP keywords all sync to the tag-only `labels` table; user-visible cross-account groupings live in `label_groups`. See the redesign for the current model. This roadmap doc is retained for historical context.
