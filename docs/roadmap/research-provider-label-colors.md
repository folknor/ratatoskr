...
> **Superseded by the folders/labels split.** See `docs/glossary/folders-labels.md` for the current model: Exchange categories, IMAP keywords, and JMAP keywords sync to the tag-only `labels` table; user-visible cross-account groupings live in `label_groups`. This roadmap doc is retained for historical context.
