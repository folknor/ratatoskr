...
Sidebar integration done (2026-03-22): `PinnedPublicFolder` type (`crates/app/src/db/types.rs`), `Db::get_pinned_public_folders()` query (`crates/app/src/db/accounts.rs`), `pinned_public_folders_section()` rendering (`crates/app/src/ui/sidebar/public_folders.rs`), loaded at boot via `Message::PinnedPublicFoldersLoaded`. Selecting a pin sets `ViewScope::PublicFolder` and routes through the existing thread-list loader: `SidebarEvent::PublicFolderSelected` (`crates/app/src/handlers/core.rs:68`) calls `load_navigation_and_threads()`, which dispatches `ViewScope::PublicFolder` to `load_public_folder_items_async()` (`crates/app/src/helpers.rs:483`) → `get_public_folder_items()` → `Thread::from_public_folder_item()`.

**Still missing (UI only):**

1. **Reading pane / thread detail when a public folder item is selected.** `handle_select_thread` short-circuits with `return Task::none()` whenever `selected_scope` is `ViewScope::PublicFolder` (`crates/app/src/handlers/core.rs:985`). Public folder items are not real threads in our DB - they live in `public_folder_items`, not `messages`/`threads` - so the existing thread-detail loader doesn't apply. Needs a dedicated detail loader that reads from `public_folder_items` and renders message bodies (likely via `GetItem` on demand for any non-cached body).
2. **Email actions on public folder items.** `handle_email_action` short-circuits the same way (`crates/app/src/handlers/commands.rs:98`). Reply / forward / delete / move are all no-ops in the public folder scope today. The EWS `CreateItem` operation is implemented in the client; what's missing is the intent → operation routing for `ViewScope::PublicFolder`.
3. **Public folder browser (lazy-load tree for browsing & pinning).** `browse_public_folders()` exists and persists to the `public_folders` table, but no UI panel calls it. There is no way for a user to discover and pin a folder from inside the app today - pins must currently be created externally (DB or harness).
4. **Compose into a mail-enabled public folder.** No compose-from-public-folder path. The reading pane's `ComposeReply` / `ComposeReplyAll` (`crates/app/src/ui/reading_pane.rs:613`) wouldn't fire anyway because of (1) - there's no selected thread/message in this scope.
...
**Update (March 2026)**: Option B was implemented in `crates/graph/src/ews/` (~1650 lines across 4 modules: `mod.rs` 637, `client.rs` 274, `parsers.rs` 586, `xml_helpers.rs` 151). Uses `quick-xml` + `reqwest` with OAuth bearer auth. Covers FindFolder, GetFolder, FindItem (with paging), GetItem, CreateItem, plus `decode_replica_list()` for PR_REPLICA_LIST binary parsing and `EwsEffectiveRights` extraction. Additionally, Autodiscover routing lives in `crates/graph/src/autodiscover.rs` (~600 lines), offline sync for pinned folders in `crates/graph/src/public_folder_sync.rs` (~720 lines), and IMAP NAMESPACE public folders in `crates/imap/src/public_folders.rs` (~735 lines). DB tables (`public_folders`, `public_folder_items`, `public_folder_pins`, `public_folder_sync_state`, `public_folder_content_routing`) live in `crates/db/src/db/schema/11_collaboration.sql` (the schema-file split landed after this section was first written - `migrations.rs` only contains a top-level comment referencing them).
...
|---|---|---|---|---|
| Minimal EWS client (quick-xml + reqwest) | Medium | Critical (enables everything) | P0 | [x] Done - `crates/graph/src/ews/` (~1600 lines, 4 modules). FindFolder, GetFolder, FindItem, GetItem, CreateItem with full XML request/response parsing. |
| EWS OAuth token acquisition (reuse existing flow) | Low | Critical | P0 | [x] Done - reuses existing Graph OAuth bearer tokens. |
| PR_REPLICA_LIST decoding | Medium | Medium (correctness) | P1 | [x] Done - `decode_replica_list()` extracts content mailbox GUIDs from binary extended property. |
| EffectiveRights permission checking | Low | High | P1 | [x] Done - `EwsEffectiveRights` struct parsed from FindFolder/GetFolder responses. |
| Autodiscover for public folder routing | Medium | Critical | P0 | [x] Done - `crates/graph/src/autodiscover.rs` (~600 lines). `discover_public_folder_routing()` for hierarchy headers, `discover_content_mailbox()` for content routing, `construct_replica_smtp()` for GUID-to-SMTP. |
| FindFolder hierarchy browsing (lazy-load) | Medium | Critical | P0 | (yellow) Backend done, UI missing - `browse_public_folders()` in `crates/graph/src/public_folder_sync.rs` walks one level at a time and persists into `public_folders`. No browser panel calls it; pins can only be created externally today. |
| FindItem for folder contents | Medium | Critical | P0 | [x] Done - FindItem with paging implemented. `sync_pinned_public_folder()` and `sync_all_pinned_folders()` in `crates/graph/src/public_folder_sync.rs` handle sync loop with local storage in `public_folder_items` table. |
| Pin/favorite folders to sidebar | Low | Critical for UX | P0 | [x] Done - Backend: `pin_public_folder()` / `unpin_public_folder()` in `crates/db/src/db/queries_extra/provider_sync_writes.rs`. Sidebar: "PUBLIC FOLDERS" section renders pinned folders with unread counts, loaded at boot (2026-03-22). |
| Thread-list load on selection | Low | Critical | P0 | [x] Done - `SidebarEvent::PublicFolderSelected` → `load_navigation_and_threads()` → `load_public_folder_items_async()` (`crates/app/src/helpers.rs:483`) → `Thread::from_public_folder_item()`. Reads from `public_folder_items`, applies thread decorations. |
| Reading pane / thread detail for PF items | Medium | Critical | P0 | [-] Not started - `handle_select_thread` short-circuits on `ViewScope::PublicFolder` (`crates/app/src/handlers/core.rs:985`). Needs a dedicated detail loader for `public_folder_items` rows + on-demand `GetItem` for bodies. |
| Email actions on PF items | Medium | High | P0 | [-] Not started - `handle_email_action` short-circuits on `ViewScope::PublicFolder` (`crates/app/src/handlers/commands.rs:98`). The `MailActionIntent` pipeline doesn't know how to map intents to EWS operations for this scope yet. |
| Offline sync for pinned folders | Medium-High | High | P1 | [x] Done - `crates/graph/src/public_folder_sync.rs` (~720 lines). Timestamp-based polling, deletion scan throttled to 1hr intervals, content routing cache in `public_folder_content_routing` table. |
| Mail-enabled folder reply/forward | Medium | High | P1 | (yellow) EWS client ready, UI not wired - `CreateItem` exists in `crates/graph/src/ews/client.rs`. Blocked behind the reading-pane gap above; reply/forward affordances need a selected message in a PF context to attach to. |
| CreateItem (post to public folder) | Low | Medium | P2 | (yellow) EWS client ready, no compose entry point - same blocker as reply/forward. |
| IMAP NAMESPACE discovery | Low | Low-Medium (self-hosted only) | P2 | [x] Done - `crates/imap/src/public_folders.rs` (~735 lines). `discover_imap_public_folders()` uses NAMESPACE + LIST, `check_folder_rights()` uses MYRIGHTS (RFC 4314). |
| IMAP shared namespace folder access | Low | Low-Medium | P2 | [x] Done - `sync_imap_public_folder()` in `crates/imap/src/public_folders.rs`. Bridges to provider-agnostic `public_folders` DB table. |

The backend critical path is complete: EWS client, Autodiscover routing, FindFolder browsing, FindItem sync, pin/unpin, offline sync, and IMAP NAMESPACE support are all implemented across `crates/graph/` and `crates/imap/`. Sidebar pin rendering and thread-list loading on selection are wired (2026-03-22). The remaining work is all in the iced app:

1. Reading pane / message-detail loader for `public_folder_items` (separate path from the regular `threads`/`messages` loader).
2. Public folder browser panel (a lazy-load tree UI on top of `browse_public_folders()`) so users can pin without leaving the app.
3. Email-action routing for `ViewScope::PublicFolder` so reply / forward / move / delete actually do something - the EWS `CreateItem` operation is ready, but `handle_email_action` short-circuits this scope today.
4. Compose-into-public-folder entry point for post items.
