...
| FindFolder hierarchy browsing (lazy-load) | Medium | Critical | P0 | 🟡 Backend done, UI missing - `browse_public_folders()` in `crates/graph/src/public_folder_sync.rs` walks one level at a time and persists into `public_folders`. No browser panel calls it; pins can only be created externally today. |
...
| Pin/favorite folders to sidebar | Low | Critical for UX | P0 | ✅ Done - Backend: `pin_public_folder()` / `unpin_public_folder()` in `crates/db/src/db/queries_extra/provider_sync_writes.rs`. Sidebar: "PUBLIC FOLDERS" section renders pinned folders with unread counts, loaded at boot (2026-03-22). |
| Thread-list load on selection | Low | Critical | P0 | ✅ Done - `SidebarEvent::PublicFolderSelected` → `load_navigation_and_threads()` → `load_public_folder_items_async()` (`crates/app/src/helpers.rs:483`) → `Thread::from_public_folder_item()`. Reads from `public_folder_items`, applies thread decorations. |
...
| Offline sync for pinned folders | Medium-High | High | P1 | ✅ Done - `crates/graph/src/public_folder_sync.rs` (~720 lines). Timestamp-based polling, deletion scan throttled to 1hr intervals, content routing cache in `public_folder_content_routing` table. |
| Mail-enabled folder reply/forward | Medium | High | P1 | 🟡 EWS client ready, UI not wired - `CreateItem` exists in `crates/graph/src/ews/client.rs`. Blocked behind the reading-pane gap above; reply/forward affordances need a selected message in a PF context to attach to. |
| CreateItem (post to public folder) | Low | Medium | P2 | 🟡 EWS client ready, no compose entry point - same blocker as reply/forward. |
| IMAP NAMESPACE discovery | Low | Low-Medium (self-hosted only) | P2 | ✅ Done - `crates/imap/src/public_folders.rs` (~735 lines). `discover_imap_public_folders()` uses NAMESPACE + LIST, `check_folder_rights()` uses MYRIGHTS (RFC 4314). |
