...
- [x] Gmail `sendAs` signature fetch - pulled on initial sync and delta sync. `sync_signatures` moved to `crates/provider-sync/src/gmail/sync/labels.rs` during the provider-sync extraction; this doc previously pointed at the old `crates/gmail/src/sync/labels.rs` location.
