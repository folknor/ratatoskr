...
- [x] Gmail `sendAs` signature fetch - pulled on initial sync and delta sync. `sync_signatures` was relocated to `crates/provider-sync/src/gmail/aux_sync.rs` (as `sync_gmail_signatures`) at the B3a-cut-gmail bifrost cutover, where it runs every kick via the bifrost Gmail runner; it previously lived in `crates/provider-sync/src/gmail/sync/labels.rs` (and earlier `crates/gmail/src/sync/labels.rs`).
- [x] Gmail bidirectional sync - local edits pushed via `update_send_as_signature` (`crates/gmail/src/api.rs:297`), conflict resolution by server HTML hash
- [x] JMAP Identity signature sync - `sync_jmap_identity_signatures` in `crates/jmap/src/signatures.rs:18`, upserts `htmlSignature`/`textSignature` keyed by `(account_id, server_id)`
