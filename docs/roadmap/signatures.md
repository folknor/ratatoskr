...
- [x] DB schema extended with sync columns (`server_id`, `server_html_hash`, `source`, `last_synced_at`, `is_reply_default`, `body_text`) in `crates/db/src/db/schema/04_compose.sql`
- [x] Gmail `sendAs` signature fetch - pulled every resident aux pass. `sync_gmail_signatures` was relocated to `crates/service/src/bifrost/gmail_signatures.rs` at B13 (it previously lived in `crates/provider-sync/src/gmail/aux_sync.rs`, itself relocated from `crates/provider-sync/src/gmail/sync/labels.rs` at the B3a-cut-gmail bifrost cutover, and earlier `crates/gmail/src/sync/labels.rs`); it now sources aliases via `AccountSettingsSurface::identities` (bifrost `identities_list`) instead of the deleted `gmail::api::list_send_as`.
- [x] Gmail signature reconciliation - local edits route through bifrost `identity_update`, with conflict resolution by server HTML hash. Production writeback remains blocked until `gmail.settings.basic` is added and connected accounts re-consent.
- [x] JMAP Identity signature sync - never wired to a caller; the dead duplicate was removed in B13 rather than documented as shipped behavior.
- [x] Inline image handling - `crates/common/src/signature_images.rs` extracts base64 data-URIs and CID references from signature HTML, deduplicates via xxh3, stores in inline image store via `store_signature_images`
- [x] Signature placement in compose - `replace_signature` in `crates/rte/src/compose.rs:229` handles all six swap cases (none↔some, swap, placeholder preservation) with unit tests. `ComposeState::active_signature_id` and `signature_separator_index` drive it; on account switch, `db_resolve_signature_for_compose` (`crates/db/src/db/queries_extra/compose.rs:358`) picks the right signature and the new-message vs reply/forward defaults come from in-editor toggles backed by `is_reply_default` (see `docs/signatures/discrepancies.md`).
- [x] Draft round-trip - `signature_id` and `signature_separator_index` are persisted columns on `local_drafts` (`04_compose.sql:62-68`), written through `SaveLocalDraftParams` (`crates/db/src/db/queries_extra/compose.rs:545`) and restored in `ComposeState::from_draft` (`crates/app/src/pop_out/compose/state.rs:196,238`). The earlier "draft restoration with signature state" gap in `docs/signatures/discrepancies.md` is closed.
...
The hand-rolled `GmailSendAs` struct and `gmail::api::{list_send_as, update_send_as_signature}` calls were deleted at B13; the reconciliation now sources/sinks through bifrost's `Identity` type (`identities_list` / `identity_update`) via `AccountSettingsSurface`. The `send_as_aliases` table has a `signature_id` FK pointing to the `signatures` table. On first sync, extract the signature HTML from the fetched `Identity` and insert it into the local `signatures` table.
...
1. **Gmail fetch on account setup** - highest value, easiest implementation. Bifrost's `Identity` (via `identities_list`) already carries the `signature_html` field.
