...
export const useThreadStore: UseBoundStore<StoreApi<ThreadState>> =
  create<ThreadState>((set, get) => ({
    threads: [],
    threadMap: new Map(),
    ...
    selectThread: (selectedThreadId: string | null) =>
      set({ selectedThreadId, selectedThreadIds: new Set() }),
    toggleThreadSelection: (id: string) =>
      set((state) => {
    ...
    selectThreadRange: (id: string) => {
      const state = get();
      const threads = state.threads;
      // Find the anchor: last selected thread or the currently viewed thread
      const anchor =
        state.selectedThreadId ?? [...state.selectedThreadIds].pop();
      if (!anchor) {
    ...
    setSearch: (query: string, threadIds: Set<string> | null) =>
