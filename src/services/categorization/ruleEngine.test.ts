...
describe("categorizeByRules", () => {
  describe("Layer 1: Gmail CATEGORY_* labels", () => {
    it("maps CATEGORY_PROMOTIONS to Promotions", () => {
      expect(
        categorizeByRules(
          input({ labelIds: ["INBOX", "CATEGORY_PROMOTIONS"] }),
        ),
      ).toBe("Promotions");
    });
    ...
    it("maps CATEGORY_SOCIAL to Social", () => {
      expect(
        categorizeByRules(input({ labelIds: ["INBOX", "CATEGORY_SOCIAL"] })),
      ).toBe("Social");
    });

    it("maps CATEGORY_UPDATES to Updates", () => {
      expect(
        categorizeByRules(input({ labelIds: ["INBOX", "CATEGORY_UPDATES"] })),
      ).toBe("Updates");
    });

    it("maps CATEGORY_FORUMS to Primary", () => {
      expect(
        categorizeByRules(input({ labelIds: ["INBOX", "CATEGORY_FORUMS"] })),
      ).toBe("Primary");
    });

    it("maps CATEGORY_PERSONAL to Primary", () => {
      expect(
        categorizeByRules(input({ labelIds: ["INBOX", "CATEGORY_PERSONAL"] })),
      ).toBe("Primary");
    });
  ...
  describe("Layer 2: Domain heuristics", () => {
    it("classifies social network domains as Social", () => {
      expect(
        categorizeByRules(
          input({ fromAddress: "notifications@facebookmail.com" }),
        ),
      ).toBe("Social");
      expect(
        categorizeByRules(input({ fromAddress: "info@linkedin.com" })),
      ).toBe("Social");
      expect(
        categorizeByRules(input({ fromAddress: "notify@twitter.com" })),
      ).toBe("Social");
    });

    it("classifies newsletter platform domains as Newsletters", () => {
      expect(
        categorizeByRules(input({ fromAddress: "author@substack.com" })),
      ).toBe("Newsletters");
      expect(
        categorizeByRules(input({ fromAddress: "campaign@mailchimp.com" })),
      ).toBe("Newsletters");
      expect(
        categorizeByRules(input({ fromAddress: "sender@beehiiv.com" })),
      ).toBe("Newsletters");
    });

    it("classifies promotional prefixes as Promotions", () => {
      expect(
        categorizeByRules(input({ fromAddress: "marketing@example.com" })),
      ).toBe("Promotions");
      expect(categorizeByRules(input({ fromAddress: "promo@shop.com" }))).toBe(
        "Promotions",
      );
      expect(categorizeByRules(input({ fromAddress: "deals@store.com" }))).toBe(
        "Promotions",
      );
    });
    ...
    it("classifies update prefixes as Updates", () => {
      expect(
        categorizeByRules(input({ fromAddress: "noreply@github.com" })),
      ).toBe("Updates");
      expect(
        categorizeByRules(input({ fromAddress: "notifications@bank.com" })),
      ).toBe("Updates");
      expect(
        categorizeByRules(input({ fromAddress: "no-reply@service.com" })),
      ).toBe("Updates");
      expect(
        categorizeByRules(input({ fromAddress: "security@company.com" })),
      ).toBe("Updates");
    });
  ...
  describe("Layer 4: Default", () => {
    it("returns Primary for regular person-to-person email", () => {
      expect(categorizeByRules(input({ fromAddress: "alice@gmail.com" }))).toBe(
        "Primary",
      );
    });
    ...
    it("returns Primary for unknown domains with normal local part", () => {
      expect(
        categorizeByRules(input({ fromAddress: "john.doe@company.com" })),
      ).toBe("Primary");
    });
  });
  ...
  describe("Priority ordering", () => {
    it("Gmail label > domain heuristic > list-unsubscribe > default", () => {
      ...
      const result = categorizeByRules(
        input({
