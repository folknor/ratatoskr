...
export class JmapProvider implements EmailProvider {
  ...
  async permanentDelete(_threadId: string, messageIds: string[]): Promise<void> {
    await invoke("jmap_permanent_delete", {
      accountId: this.accountId,
      emailIds: messageIds,
    });
  }
  ...
  async removeLabel(threadId: string, labelId: string): Promise<void> {
    await invoke("jmap_remove_label", {
      accountId: this.accountId,
      threadId,
      labelId,
    });
  ...
  async sendMessage(rawBase64Url: string, threadId?: string): Promise<{ id: string }> {
    return invoke<{ id: string }>("jmap_send_email", {
      accountId: this.accountId,
  ...
  async createDraft(rawBase64Url: string, threadId?: string): Promise<{ draftId: string }> {
    return invoke<{ draftId: string }>("jmap_create_draft", {
      accountId: this.accountId,
      rawBase64url: rawBase64Url,
      threadId,
    });
  ...
  async updateDraft(
    ...
    return invoke<{ draftId: string }>("jmap_update_draft", {
      accountId: this.accountId,
      draftId,
      rawBase64url: rawBase64Url,
      threadId,
  ...
  async deleteDraft(draftId: string): Promise<void> {
    await invoke("jmap_delete_draft", {
      accountId: this.accountId,
      draftId,
    });
  }
