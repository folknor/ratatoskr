...
export function PhishingBanner({
  ...
  return (
    <div
      className={`mx-4 my-2 px-3 py-2.5 rounded-lg border ${bgClass} flex items-center gap-3`}
    >
      <ShieldAlert size={18} className={`shrink-0 ${iconClass}`} />
      <div className="flex-1 min-w-0">
        <p className={`text-xs font-medium ${textClass}`}>
          {isHigh
            ? t("phishingBanner.highRisk")
            : t("phishingBanner.suspicious")}{" "}
          {t("phishingBanner.linksDetected")}
        </p>
        <p className="text-xs text-text-tertiary mt-0.5">
          {t("phishingBanner.suspiciousLink", {
            count: scanResult.suspiciousLinkCount,
          })}{" "}
