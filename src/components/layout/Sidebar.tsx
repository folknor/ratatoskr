          ...
          return (
            <div key={item.id}>
              <DroppableNavItem
                ...
                title={
                  collapsed
                    ? t(NAV_ITEM_I18N_KEY[item.id] ?? item.id)
                    : undefined
                }
              >
                {() => (
                  ...
                  {CATEGORY_ITEMS.map((cat) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="flex-1 truncate">
                          {t(CATEGORY_I18N_KEY[cat.id] ?? cat.id)}
                        </span>
                      </button>
                    );
                  })}
        ...
        {showSmartFolders && (smartFolders.length > 0 || !collapsed) && (
          <>
            ...
            {smartFolders.map((folder) => {
              ...
              const displayName = folder.isDefault
                ? t(folder.id, { defaultValue: folder.name })
                : folder.name;
              return (
                <button
                  key={folder.id}
              ...
              >
                {labelsExpanded ? (
                  ...
                  <>
                    ...
                    <span>
                      {t("nMore", {
                        count: labels.length - LABELS_COLLAPSED_COUNT,
                      })}
                    </span>
                  </>
                )}
        ...
        fields={[
          {
            key: "name",
            label: t("common:name"),
            placeholder: t("smartFolderName"),
          },
          {
            key: "query",
            label: t("searchQuery"),
            placeholder: t("smartFolderQuery"),
          },
        ]}
      />
