...
**Programmatic scrolling uses widget operations, not `scrollable` methods.** The fork exposes `iced::widget::operation::{snap_to, snap_to_end, scroll_to}` (re-exported from `runtime::widget::operation`). Each returns a `Task<T>` ready to return from `update()`. Give the `Scrollable` a stable id via `.id("my-scrollable".to_string())`, then call e.g. `iced::widget::operation::snap_to_end::<Message>("my-scrollable".to_string())`. There is no method on `Scrollable` itself for this - go through the operation, not the widget.
