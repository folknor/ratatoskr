...
pub async fn sync_shared_account(
    ...
    if email_state.is_none() {
        log::info!("Shared JMAP account {jmap_account_id}: no state found, running initial sync");
        ...
        log::info!("Shared JMAP account {jmap_account_id}: state found, running delta sync");
...
async fn shared_initial_sync(ctx: &SyncCtx<'_>) -> Result<(), String> {
    ...
    let since = chrono::Utc::now() - chrono::Duration::days(SHARED_ACCOUNT_INITIAL_SYNC_DAYS);
    ...
    log::info!("[JMAP] Shared account {jmap_id} initial sync complete: {fetched} messages");
...
async fn shared_delta_sync(ctx: &SyncCtx<'_>) -> Result<SyncResult, String> {
    ...
    loop {
        ...
        let changes = response.get(&handle).map_err(|e| {
            let msg = e.to_string();
        ...
        if !ids_to_fetch.is_empty() {
            for chunk in ids_to_fetch.chunks(BATCH_SIZE) {
                let emails = fetch_email_batch_for(ctx.client, Some(jmap_id), chunk).await?;
                let parsed = parse_email_batch(&emails, &mailbox_map)?;
