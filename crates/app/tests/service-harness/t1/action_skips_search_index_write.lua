-- description: trash action mutates locally without writing search index
-- ceiling: 90s
...
local dir = harness.data_dir("t1_action_skips_search_index_write")
...
local thread, thread_err = client:request("TestSeedThread", {
    ...
    thread_id = "m4-search-thread",
    label_ids = { "INBOX" },
})
harness.assert(thread_err == nil, "seed thread failed")

local before, before_err = client:request("TestCounterRead", {
    counter = "search.write",
})
harness.assert(before_err == nil, "counter read before failed")
...
local after, after_err = client:request("TestCounterRead", {
    counter = "search.write",
})
harness.assert(after_err == nil, "counter read after failed")
harness.assert_eq(after.value, before.value, "search.write counter changed")
...
harness.assert(has_label(read, "TRASH"), "trash label missing")
