-- description: CalDAV calendar RSVP (Accept) writeback round-trip
-- expected: pass
...
-- for the three-plane proof structure. CalDAV RSVP is a scheduling
-- round-trip: bifrost GETs the event .ics, POSTs an iTIP REPLY to the
-- principal's schedule-outbox, then PUTs the event back with the self
-- attendee's PARTSTAT patched (matched against the principal's
-- calendar-user-address-set mailto = account.name = test@example.com).
...
local dir = harness.data_dir("sync_calendar_rsvp_caldav")
...
local account, account_err = client:request("TestSeedAccount", {
    email = "sync-calendar-rsvp-caldav@example.test",
    display_name = "Sync Calendar RSVP CalDAV",
    account_name = "Sync Calendar RSVP CalDAV",
    provider = "caldav",
...
local invite = event_by_google_event_id(initial.calendar_events, "caldav:ev-rsvp-001")
...
local rsvped_row = event_by_google_event_id(after_rsvp.calendar_events, "caldav:ev-rsvp-001")
...
-- Plane 2: bifrost issued the GET + schedule-outbox POST + PUT the
-- CalDAV RSVP requires. The schedule-outbox POST is recorded twice (the
-- generic CalDAV dispatch logs every request, and the outbox handler
-- logs the scheduling reply), so it reads as 2; the GET pre-read and
-- the PARTSTAT-carrying PUT are one each.
local requests = harness.mock_requests(admin_endpoint)
harness.assert_eq(
    harness.request_count(
        requests,
        "caldav",
        "GET /calendars/account-1/cal-work/ev-rsvp-001.ics"
    ),
    1,
    "expected exactly one CalDAV RSVP pre-read GET"
)
harness.assert_eq(
    harness.request_count(
        requests,
        "caldav",
        "POST /calendars/account-1/outbox/"
    ),
    2,
    "expected the schedule-outbox POST (dispatch + handler log = 2)"
)
harness.assert_eq(
    harness.request_count(
        requests,
        "caldav",
        "PUT /calendars/account-1/cal-work/ev-rsvp-001.ics"
    ),
    1,
    "expected exactly one CalDAV RSVP PUT-back"
)

-- Plane 3 (load-bearing): a follow-up sync re-reads the event; the self
-- attendee's persisted PARTSTAT now reads accepted, and no attendee is
-- left at needs-action.
...
local final_invite = event_by_google_event_id(final.calendar_events, "caldav:ev-rsvp-001")
