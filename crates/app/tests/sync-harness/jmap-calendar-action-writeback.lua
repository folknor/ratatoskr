-- description: JMAP calendar create, update, delete, and writeback round-trip
-- expected: pass
...
local dir = harness.data_dir("sync_calendar_actions_jmap_crud")
...
local account, account_err = client:request("TestSeedAccount", {
    email = "sync-calendar-actions-jmap@example.test",
    display_name = "Sync Calendar Actions JMAP",
    account_name = "Sync Calendar Actions JMAP",
    provider = "jmap",
})
...
-- B7b owns write PROPAGATION, asserted above: after the delta resync the
-- created event is present and the deleted one is absent, with exactly two
-- rows (no transient duplicate). We deliberately do NOT assert the delta
-- issued an incremental CalendarEvent/changes here: because a B7b write goes
-- through a separate bifrost Account session, the next read-sync currently
-- falls back to a full CalendarEvent re-query rather than an incremental
-- /changes call. That incremental-vs-full fetch-strategy behaviour is B7a
-- read-sync territory, not B7b action-writeback; it is tracked as a read-sync
-- (B7a-scope) follow-up and must not gate this action-writeback script.
