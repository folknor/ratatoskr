-- description: CalDAV calendar create, update, delete, and writeback round-trip
-- expected: pass
...
local dir = harness.data_dir("sync_calendar_actions_caldav_crud")
...
local account, account_err = client:request("TestSeedAccount", {
    email = "sync-calendar-actions-caldav@example.test",
    display_name = "Sync Calendar Actions CalDAV",
    account_name = "Sync Calendar Actions CalDAV",
    provider = "caldav",
...
-- Id-stability (spec § 4.2 / R1 finding 2): the id stored at create time must
-- equal the read-sync dedup key. CalDAV is the one provider where the
-- create-time href and the read-sync dedup key (`caldav:{uid}`) are built
-- differently, so a mismatch would surface here as a duplicate on re-sync.
harness.clear_mock_requests(admin_endpoint)
...
harness.assert(
    event_by_google_event_id(after_delta.calendar_events, "caldav:ev-002") == nil,
    "deleted review returned after delta"
)
harness.assert(
    event_by_summary(after_delta.calendar_events, "Harness CalDAV created") ~= nil,
