...
-- ceiling: 120s
--
-- MDN (read-receipt) round-trip gate for JMAP (B5-GATES). The consumer hydrates
-- the whole-message raw RFC822 (engine.open_raw_rfc822 -> the mock's
-- /jmap/download serving the full message), sees the fixture message's
-- Disposition-Notification-To, and sets mdn_requested = 1. With an "always"
-- read-receipt policy, marking the thread read dispatches the RFC 8098 MDN via
-- engine.send_raw_message -> JMAP Email/import + EmailSubmission/set. The gate
-- resyncs and asserts the MDN round-tripped into SENT addressed to the original
-- sender (the JMAP analogue of gmail-mdn.lua's SENT round-trip).
...
local dir = harness.data_dir("sync_jmap_mdn")
...
local account, account_err = client:request("TestSeedAccount", {
    ...
    display_name = "Sync JMAP MDN",
    account_name = "Sync JMAP MDN",
    provider = "jmap",
