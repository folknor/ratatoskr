-- description: B8 JMAP contact update/delete use engine passthroughs
-- expected: pass
...
local client, spawn_err = harness.spawn(harness.data_dir("b8_writeback_jmap"))
harness.assert(spawn_err == nil, "spawn failed")
local _, ready_err = client:request("BootReady")
harness.assert(ready_err == nil, "boot.ready failed")

local account, seed_err = client:request("TestSeedAccount", { email = "b8-w-jmap@example.test", provider = "jmap" })
...
local saved, save_err = client:request("contacts.contact_save_with_writeback", {
    ...
    phone = "+1 555 0101", company = "Bifrost", notes = "JMAP writeback",
