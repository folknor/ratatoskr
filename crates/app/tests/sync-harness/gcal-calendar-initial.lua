-- description: Google Calendar initial sync imports the small calendar fixture
-- expected: pass
...
local dir = harness.data_dir("sync_gcal_calendar_initial")
...
local account, account_err = client:request("TestSeedAccount", {
    email = "sync-gcal-calendar@example.test",
    display_name = "Sync Google Calendar",
    account_name = "Sync Google Calendar",
    provider = "gmail_api",
})
...
harness.assert(
    personal_event_requests >= 1,
    "Google Calendar sync did not list Personal events"
)
