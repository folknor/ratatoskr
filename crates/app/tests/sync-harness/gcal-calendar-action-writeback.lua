-- description: Google Calendar create, update, delete, and writeback round-trip
-- expected: pass
...
local function request_body(requests, command)
    for _, request in ipairs(requests) do
        if request.protocol == "gcal" and request.command == command then
...
local dir = harness.data_dir("sync_calendar_actions_gcal_crud")
...
local account, account_err = client:request("TestSeedAccount", {
    email = "sync-calendar-actions-gcal@example.test",
    display_name = "Sync Calendar Actions Google",
    account_name = "Sync Calendar Actions Google",
    provider = "gmail_api",
})
...
local update_input = {
    title = "Google Standup moved",
    description = "Moved by harness mutation coverage",
    location = "Conf Room E",
...
harness.assert_eq(final_standup.summary, "Google Standup moved", "updated summary")
harness.assert_eq(final_standup.location, "Conf Room E", "updated location")
...
local final_created = event_by_summary(final.calendar_events, "Harness Google created")
...
local create_body = request_body(
    requests,
    "POST /calendar/v3/calendars/cal-work/events"
)
harness.assert(create_body ~= nil, "missing create request body")
harness.assert_eq(create_body.summary, "Harness Google created", "create summary")
harness.assert_eq(create_body.location, "Focus Room", "create location")
harness.assert_eq(create_body.start.dateTime, "2026-02-03T10:00:00Z", "create start")

local patch_body = request_body(
    requests,
    "PATCH /calendar/v3/calendars/cal-work/events/ev-001"
)
harness.assert(patch_body ~= nil, "missing patch request body")
harness.assert_eq(patch_body.summary, "Google Standup moved", "patch summary")
harness.assert_eq(patch_body.location, "Conf Room E", "patch location")
harness.assert_eq(patch_body.start.dateTime, "2026-01-15T10:00:00Z", "patch start")
harness.assert(
    harness.request_count(
        requests,
        "gcal",
        "DELETE /calendar/v3/calendars/cal-work/events/ev-002"
    ) >= 1,
    "missing Google Calendar delete request"
)
...
harness.assert(
    ...
    "post-action sync did not call Work calendar events"
)
