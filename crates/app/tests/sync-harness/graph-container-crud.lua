-- description: Graph mail-folder create/rename/move/delete round-trips through engine.container_*
-- expected: pass
-- fixture: graph-initial.toml
-- protocol: graph
-- ceiling: 120s
--
-- B6b container-CRUD gate (Graph). The folder CRUD action handlers dispatch
-- through engine.container_create / container_rename / container_move /
-- container_delete; a follow-up resync's containers_list must reflect each
-- step on the server.
...
local dir = harness.data_dir("sync_graph_container_crud")
...
local account, account_err = client:request("TestSeedAccount", {
    email = "sync-graph-crud@example.test",
    display_name = "Sync Graph CRUD",
    account_name = "Sync Graph CRUD",
    provider = "graph",
})
