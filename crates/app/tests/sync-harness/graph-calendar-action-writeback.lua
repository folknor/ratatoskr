-- description: Graph calendar create, update, delete, and writeback round-trip
-- expected: pass
...
local dir = harness.data_dir("sync_calendar_actions_graph_crud")
...
local account, account_err = client:request("TestSeedAccount", {
    email = "sync-calendar-actions-graph@example.test",
    display_name = "Sync Calendar Actions Graph",
    account_name = "Sync Calendar Actions Graph",
    provider = "graph",
})
...
-- B7b addresses Graph events at the CALENDAR-scoped route
-- (`/v1.0/me/calendars/{cal}/events/{id}`), which the mock now serves.
-- The mock records each such request twice: the `log_request`
-- middleware logs the raw calendar-scoped path (detail carries the
-- query only, no body), and the inner event handler logs the canonical
-- mailbox-scoped path with the parsed body. So the calendar-scoped
-- command is the route assertion, and the field-content assertion reads
-- the body off the handler's (mailbox-scoped) record - the only place
-- the body is captured.
harness.assert(
    harness.request_count(
        requests,
        "graph",
        "PATCH /v1.0/me/calendars/cal-work/events/ev-001"
    ) >= 1,
    "update did not use the calendar-scoped Graph event route"
)
...
harness.assert(
    harness.request_count(
        requests,
        "graph",
        "DELETE /v1.0/me/calendars/cal-work/events/ev-002"
    ) >= 1,
    "delete did not use the calendar-scoped Graph event route"
)
...
-- the DB state asserted above. It goes through a fresh bifrost Account
-- session (the B7b write path), which does not carry the previous
-- session's `$deltatoken`, so the resync full-refetches via
-- `calendarView` rather than issuing an incremental `calendarView/delta`
-- call. That incremental-vs-full fetch-strategy is B7a read-sync
-- territory, not B7b action-writeback; assert only that the resync
-- read the Work calendar (either form), via the shared prefix.
