-- description: Graph contact deltas import scripted new/change/delete steps
-- expected: pass
-- fixture: graph-contacts-incremental.lua
-- protocol: graph
-- ceiling: 120s

local CONTACT_DELTA = "GET /v1.0/me/contactFolders/cf-default/contacts/delta"
...
local function run_until_contact_delta(client, endpoint, account_id, label)
    -- 20 is the production Graph contact-delta cadence.
    for i = 1, 20 do
...
local dir = harness.data_dir("sync_graph_contacts_incremental")
...
local account, account_err = client:request("TestSeedAccount", {
    email = "sync-graph-contacts-delta@example.test",
    display_name = "Sync Graph Contacts Delta",
    account_name = "Sync Graph Contacts Delta",
    provider = "graph",
})
...
harness.assert_eq(carol.server_id, "contact-003", "Carol server id")
...
local robert = contact_by_email(after_change.contacts, "robert@work.example")
harness.assert(robert ~= nil, "Robert work contact missing")
harness.assert_eq(robert.display_name, "Robert Bell", "Robert work display name")
harness.assert_eq(robert.server_id, "contact-002", "Robert work server id")
...
harness.assert(contact_by_email(after_delete.contacts, "robert@work.example") ~= nil, "Robert lost")
