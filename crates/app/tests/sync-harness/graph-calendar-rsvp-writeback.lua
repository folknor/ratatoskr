-- description: Graph calendar RSVP (Accept) writeback round-trip
-- expected: pass
...
-- for the three-plane proof structure. Graph RSVP is a server-side
-- action: bifrost POSTs to the calendar-scoped
-- `/v1.0/me/calendars/{cal}/events/{id}/accept` route (no client-side
-- self-match; the mock's accept handler sets the caller's own attendee
-- - matched by `account.name` = test@example.com - to accepted). The
-- mock records that POST under its canonical mailbox-scoped command
-- string.
...
local dir = harness.data_dir("sync_calendar_rsvp_graph")
...
local account, account_err = client:request("TestSeedAccount", {
    email = "sync-calendar-rsvp-graph@example.test",
    display_name = "Sync Calendar RSVP Graph",
    account_name = "Sync Calendar RSVP Graph",
    provider = "graph",
})
...
-- Plane 2: bifrost issued exactly one accept POST. The mock logs the
-- calendar-scoped accept under its canonical mailbox-scoped command.
local requests = harness.mock_requests(admin_endpoint)
harness.assert_eq(
    harness.request_count(
        requests,
        "graph",
        "POST /v1.0/me/events/ev-rsvp-001/accept"
    ),
    1,
    "expected exactly one Graph RSVP accept POST"
)
