-- description: JMAP steady-state sync uses changes endpoints without duplicating mail
-- expected: pass
...
local account, account_err = client:request("TestSeedAccount", {
    email = "sync-jmap-delta@example.test",
    display_name = "Sync JMAP Delta",
    account_name = "Sync JMAP Delta",
    provider = "jmap",
})
...
-- The resident auxiliary pass fires RESIDENT_AUX_INITIAL_DELAY (5s) after
-- attach - AFTER this script's clear point. Without this quiesce the script
-- passed only because its whole run finished before the 5s tick; any slowdown
-- pushing the measured window past 5s counted aux requests against the
-- max_delta = 0 budget. Wait out the aux start, then wait for the request log
-- to go quiet, so the window below measures the delta kick and nothing else.
harness.sleep(6000)
local quiesce = #harness.mock_requests(jmap_endpoint)
for _ = 1, 20 do
    harness.sleep(500)
    local now = #harness.mock_requests(jmap_endpoint)
    if now == quiesce then
        break
    end
    quiesce = now
...
-- Re-snapshot AFTER the aux pass settled: the delta comparison below must be
-- against the settled state, not the mid-boot one.
