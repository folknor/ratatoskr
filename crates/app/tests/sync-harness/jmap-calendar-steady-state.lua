-- description: JMAP Calendar steady-state pull remains bounded and correct
-- expected: pass
...
local dir = harness.data_dir("sync_jmap_calendar_steady_state")
local client, err = harness.spawn(dir)
harness.assert(err == nil, "spawn failed")
harness.assert(client:request("BootReady").ready, "boot.ready returned ready=false")
local account = client:request("TestSeedAccount", { email = "jmap-calendar-steady@example.test", display_name = "JMAP Calendar Steady", account_name = "JMAP Calendar Steady", provider = "jmap" })
...
harness.assert(harness.request_count(requests, "jmap", "CalendarEvent/query") >= 1, "steady pull missed CalendarEvent/query")
