-- description: IMAP sync triggers PrefetchRuntime; attachment bytes land in PackStore via folder-batched FETCH BODY[part]
-- expected: pass
-- fixture: imap-attach.toml
-- protocol: imap
-- ceiling: 120s

-- Phase 7 (attachments roadmap) provider parity: IMAP's prefetch path
-- batches one LOGIN + SELECT per (account, folder) and issues
-- `fetch_attachment_on_selected` per item. This is the end-to-end
-- guard that the folder-batched fetch path actually populates
-- `attachments.content_hash` and writes a pack file by the time
-- `prefetch.completed` fires.
--
-- Depends on saehrimnir serving attachments over IMAP (BODYSTRUCTURE
-- + BODY[part]) when [[email.attachment]] is present in the fixture.
...
local dir = harness.data_dir("sync_imap_attachment_prefetch")
...
local account, account_err = client:request("TestSeedAccount", {
    email = "sync-imap-prefetch@example.test",
    display_name = "Sync IMAP Prefetch",
    account_name = "Sync IMAP Prefetch",
    provider = "imap",
})
...
-- Folder-batching contract: exactly one LOGIN + one SELECT INBOX from
-- the prefetch path on top of whatever sync itself issued. We can't
-- tell prefetch's IMAP traffic apart from sync's at the wire level
-- here (both share the connection); the request-count breakdown is
-- informational for the summary, not a hard assertion.
