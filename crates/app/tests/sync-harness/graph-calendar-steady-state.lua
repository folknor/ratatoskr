-- description: Graph Calendar steady-state pull remains bounded and correct
-- expected: pass
...
local dir = harness.data_dir("sync_graph_calendar_steady_state")
local client, err = harness.spawn(dir)
harness.assert(err == nil, "spawn failed")
harness.assert(client:request("BootReady").ready, "boot.ready returned ready=false")
local account = client:request("TestSeedAccount", { email = "graph-calendar-steady@example.test", display_name = "Graph Calendar Steady", account_name = "Graph Calendar Steady", provider = "graph" })
...
harness.assert(harness.request_count(requests, "graph", "GET /v1.0/me/calendars/cal-work/calendarView") >= 1, "steady pull missed Work")
