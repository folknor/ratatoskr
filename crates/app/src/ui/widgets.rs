...
fn highlighted_text_body<'a, M: 'a>(body: &'a str, terms: &'a [String]) -> Element<'a, M> {
    ...
    // Render as a wrapping row of text spans.
    // iced doesn't support inline rich text with mixed backgrounds in a
    // single `text` widget, so we use `iced::widget::rich_text` with spans.
    // Fallback: render as a column of text widgets per line, with inline
    // highlighting per line. For simplicity, use a single text widget per
    // segment in a wrapping row.
    let mut col = column![].spacing(0);

    // Process line by line for proper wrapping
    for line in body.split('\n') {
        ...
        for &(seg_start, seg_end, is_match) in &segments {
            // Skip segments outside this line
