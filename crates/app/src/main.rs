...
use ui::calendar::{
    CalendarEventData, CalendarMessage, CalendarOverlay, CalendarState, CalendarView,
    EventField,
};
...
impl App {
    fn handle_calendar(&mut self, cal_msg: CalendarMessage) -> Task<Message> {
        match cal_msg {
...
fn db_event_to_calendar_data(ev: &db::CalendarEvent) -> CalendarEventData {
    use chrono::TimeZone;
...
fn calendar_data_to_timestamp(date: NaiveDate, hour: u32, minute: u32) -> i64 {
    use chrono::TimeZone;
    ...
    fn handle_status_bar_event(&mut self, event: StatusBarEvent) -> Task<Message> {
        match event {
            StatusBarEvent::RequestReauth { account_id } => {
                // Re-authentication UI is not yet implemented. Show a
                // status message so the user knows the click was received,
