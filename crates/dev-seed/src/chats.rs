...
use crate::templates;
use crate::threads::{PendingBody, SeedStats};
...
pub fn seed_chats(
    ...
    pools: &PeoplePools,
    locale_mode: &str,
    chats_per_account: u32,
    bodies: &mut Vec<PendingBody>,
    stats: &mut SeedStats,
) -> Result<(), String> {
    if chats_per_account == 0 || accounts.is_empty() {
        return Ok(());
    }
    ...
    for acc in accounts {
        let mut picked_for_account: u32 = 0;
        ...
        let attempt_limit = chats_per_account.saturating_mul(20).max(pool_cap);

        while picked_for_account < chats_per_account && attempts < attempt_limit {
            attempts += 1;
            ...
            // Don't reuse a partner across accounts, and don't pick the
            // user's own email as their chat partner.
            ...
            for _ in 0..n_threads {
                ...
                // Thread start: 1-60 days ago.
                let days_back = rng.random_range(1..=60);
                let hours_back = rng.random_range(0..24);
                let mins_back = rng.random_range(0..60);
                let thread_start =
                    now - (days_back * 86_400 + hours_back * 3_600 + mins_back * 60) as i64;
                ...
                for mi in 0..n_msgs {
                    ...
                    // Messages 30 min - 8 h apart, monotonically forward.
                    let gap = i64::from(rng.random_range(30..480)) * 60;
                    let msg_date = thread_start + i64::from(mi) * gap;
