...
fn canonical_recurrence_slot(timestamp: i64, tz: &RecurrenceTz, all_day: bool) -> String {
    let Some(naive) = tz.naive(timestamp) else {
        // Resolution failed (timestamp out of the representable range).
        // Return a sentinel that won't collide with any real iCal canonical
        // form; the worst case is one missed dedup on a pathological event.
...
/// only if the resulting civil datetime or zone resolution overflows
...
fn parse_until_date(val: &str) -> Option<Until> {
    ...
    // a malformed UNTIL to manifest. 9999 is the outer representable year;
    // values past it saturate and silently land elsewhere.
