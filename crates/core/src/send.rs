...
//! - [`build_mime_message_base64url`] — MIME message as base64url string (Gmail)
//! - Draft lifecycle helpers — `mark_draft_sending`, `mark_draft_sent`, `mark_draft_failed`
//! - [`process_finalized_drafts`] — picks up 'finalized' drafts and sends them
//!
//! Provider-specific send logic lives in each provider crate. This module
//! produces the raw RFC 2822 bytes that providers consume.
...
pub fn build_mime_message_base64url(
    req: &SendRequest,
...
pub async fn mark_draft_sending(
    db: &DbState,
    draft_id: String,
) -> Result<(), String> {
        ...
        let rows = conn
            .execute(
                "UPDATE local_drafts SET sync_status = 'sending' \
                 WHERE id = ?1 AND sync_status IN \
                 ('pending', 'synced', 'finalized', 'failed')",
                params![draft_id],
            )
...
/// Fetch all finalized drafts for a given account that are ready to send.
pub async fn get_finalized_drafts(
    db: &DbState,
    account_id: String,
) -> Result<Vec<FinalizedDraft>, String> {
    db.with_conn(move |conn| {
        let mut stmt = conn
            .prepare(
                "SELECT id, account_id, to_addresses, cc_addresses, \
                 bcc_addresses, subject, body_html, \
                 reply_to_message_id, thread_id, from_email \
                 FROM local_drafts \
                 WHERE account_id = ?1 AND sync_status = 'finalized' \
...
/// A finalized draft row, ready to be built into a MIME message and sent.
#[derive(Debug, Clone)]
...
impl FinalizedDraft {
    /// Convert to a [`SendRequest`], producing plain-text from HTML.
    pub fn to_send_request(&self) -> SendRequest {
        let html = self.body_html.clone().unwrap_or_default();
        let plain = crate::db::queries_extra::compose::html_to_plain_text(&html);
        let from = self
            .from_email
            .clone()
        ...
        SendRequest {
            ...
            from,
            to: csv_to_vec(self.to_addresses.as_deref()),
            cc: csv_to_vec(self.cc_addresses.as_deref()),
            bcc: csv_to_vec(self.bcc_addresses.as_deref()),
            subject: self.subject.clone(),
            body_html: html,
            body_text: plain,
            attachments: Vec::new(),
            in_reply_to: self.reply_to_message_id.clone(),
            references: None,
...
/// Split a comma-separated address string into a `Vec<String>`.
...
mod tests {
    ...
    fn test_build_mime_with_reply_headers() {
        ...
        req.references = Some(
            "<root@example.com> <original@example.com>".to_string(),
        );
    ...
    fn test_build_mime_base64url() {
        let req = minimal_request();
        let encoded =
            build_mime_message_base64url(&req).expect("should encode");
    ...
    fn test_csv_to_vec() {
        assert_eq!(
            csv_to_vec(Some("a@b.com, c@d.com")),
            vec!["a@b.com", "c@d.com"]
        );
