...
(`brokkr service-test`, `service-suite`, `sync`). Tests are
...
**Enforcement:** `ViewScope` enum (`AllAccounts`, `Account`, `SharedMailbox`, `PublicFolder`) in `crates/core/src/scope.rs`. The sidebar stores `selected_scope: ViewScope` as the single source of truth. `fire_navigation_load()` and `load_threads_for_current_view()` dispatch on the enum - shared mailboxes route through the dedicated `get_threads_for_shared_mailbox{,_starred,_snoozed,_label_group}` / `get_shared_mailbox_navigation` functions, public folders route through the shared `get_threads_for_namespace` query family (public folders have no sub-navigation), personal accounts use `AccountScope`-based queries. Every non-personal row carries immutable `threads.namespace_kind` and `namespace_id`; personal queries filter `namespace_kind IS NULL`. Public-folder items are ordinary `messages` and `threads`, never pseudo-items. The action batch preflight rejects public-folder operations, unsupported JMAP shared mutations, rights-denied shared mutations, and cross-namespace moves before a local write.
