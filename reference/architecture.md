...
**Provider crates** (`gmail`, `jmap`, `graph`, `imap`) each implement the `ProviderOps` trait (`common/src/ops.rs`). No provider-specific logic should leak into app or core beyond the trait surface. Service-side sync implementations live in `provider-sync`, which owns `SyncProviderCtx`, `ProviderSyncOps`, and the per-provider orphan impls that carry writer-half state - for Gmail, Graph, and IMAP. JMAP sync has been cut over to the bifrost change-stream consumer (`crates/service/src/bifrost/`); its `provider-sync` sync impl is deleted and `run_sync` routes JMAP accounts to the engine path while every other provider stays on `provider-sync` via the coexistence dispatch. JMAP's `ProviderOps` action methods still live in the `jmap` crate. See the bifrost-migration B3a-cut-jmap landing commit.
...
**Partial-delta membership merge vs full-thread replace** - Thread folder and raw-label writes follow one of two provider-local paths depending on whether the provider gives full-thread or partial-thread snapshots, both routed through `crates/provider-sync/src/thread_membership.rs`. Gmail storage and the IMAP thread-store pass (`crates/provider-sync/src/imap/thread_store.rs`) call `replace_thread_membership_from_full_coverage` - destructive replace is safe because the caller has every message of the thread. The Graph sync module calls `replace_message_membership_and_recompute`, which atomically replaces the per-message rows for one message and recomputes the thread aggregate from the union. JMAP uses the folder-only variant (`replace_message_folders_and_recompute`, whose labels are keyword-shaped and route through the keyword recompute), but since the JMAP cutover (B3a-cut-jmap) that call lives in the bifrost consumer's write txn (`crates/service/src/bifrost/consumer/write.rs`), reached via the `provider_sync::consumer_support` facade, not in a `provider-sync` JMAP sync module (which is deleted). Neither helper exposes a separable "write rows" step from the recompute, so a contributor cannot forget the second half. `db::queries_extra::thread_persistence` and `message_membership` expose only raw row insert/delete primitives; provider-semantic orchestration lives in `provider-sync` (Gmail/Graph/IMAP) or the bifrost consumer (JMAP), where the provider's coverage shape is known. Same-client moves continue to mutate `thread_folders` locally before dispatching to the provider; same-client label actions write `pending_thread_label_intents` and let provider truth catch up.
