...
// ---------------------------------------------------------------------------
// otherContacts sync (public entry point)
// ---------------------------------------------------------------------------
...
pub async fn sync_google_other_contacts(
    ...
    match existing_token {
        Some(token) => {
            match other_contacts_incremental_sync(client, account_id, db, &token).await {
                Ok(result) => Ok(result),
                Err(e)
                    if e.contains("410")
                        || e.contains("GONE")
                        || e.contains("syncToken") =>
                {
                    ...
                    other_contacts_full_sync(client, account_id, db).await
                }
                Err(e) => Err(e),
            }
        }
        None => other_contacts_full_sync(client, account_id, db).await,
    }
}

// ---------------------------------------------------------------------------
// otherContacts full sync
// ---------------------------------------------------------------------------

async fn other_contacts_full_sync(
    client: &GmailClient,
    account_id: &str,
    db: &DbState,
) -> Result<SyncContactsResult, String> {
...
// ---------------------------------------------------------------------------
// otherContacts incremental sync
// ---------------------------------------------------------------------------

async fn other_contacts_incremental_sync(
    client: &GmailClient,
    account_id: &str,
    db: &DbState,
    sync_token: &str,
...
// ---------------------------------------------------------------------------
// otherContacts persistence helpers (run inside a transaction)
// ---------------------------------------------------------------------------
