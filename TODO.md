...
- [x] **Wire email actions to DB** — *(2026-03-23)* All email actions now perform local DB operations: Archive (remove INBOX label), Trash (remove INBOX + add TRASH), PermanentDelete (delete_thread), ToggleSpam (remove INBOX + add SPAM), MoveToFolder (remove INBOX + add target), Snooze (remove INBOX + set is_snoozed/snooze_until), ToggleRead/TogglePin/ToggleMute (optimistic UI toggle + set_thread_* core functions). Provider write-back for these actions is still pending — currently only ToggleStar and AddLabel/RemoveLabel have provider sync. Undo tokens are created but compensation logic still deferred until provider write-back is wired.
