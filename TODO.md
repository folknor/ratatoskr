...
- [ ] **Palette NavigateToLabel untyped at boundary** — `dispatch_parameterized` defaults all cross-account label navigation to `ProviderFolder`. If a user navigates to a tag via the palette, it'll be mistyped. Fix: encode `label_kind` into the `OptionItem.id` in `get_all_labels_cross_account()` (`crates/app/src/db/palette.rs`), then decode in `split_cross_account_id()` to construct the correct `SidebarSelection::ProviderFolder` or `::Tag`.
- [ ] **Potential tag duplication in navigation state** — When scoped to a single account, `build_account_labels()` returns tags as `FolderKind::AccountTag` (per-account, native ID) and `build_all_account_tags()` also returns tags as `FolderKind::AccountTag` (cross-account, `tag:{name}` ID). Both appear in the "LABELS" section via the same filter. Could produce duplicate entries with different IDs for the same label. Investigate whether both builders run when single-account scoped.
- [ ] **`App.navigation_target` is vestigial** — After the `SidebarSelection` refactoring, `navigation_target` only holds `Chat` (Search/PinnedSearch are handled by `thread_list.mode` and `active_pinned_search`). Could be simplified to `active_chat: Option<String>` and the `NavigationTarget` enum shrunk further.
