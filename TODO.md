...
- [ ] **Snooze resurface mechanism** — Snooze sets `is_snoozed = 1` + `snooze_until` timestamp + removes INBOX label (`core::actions::snooze`), and unsnooze restores inbox + clears snooze state (`core::actions::unsnooze`). The Snoozed virtual folder queries `threads.is_snoozed = 1`. But **nothing triggers unsnooze when the time arrives.** `db_get_snoozed_threads_due(db, now)` exists in `core::db::queries_extra::misc.rs` and returns threads where `snooze_until <= now`, but it is never called — no timer, no tick, no boot-time check. Snoozed threads stay snoozed forever. Fix: add a periodic `SnoozeTick` subscription (e.g., every 60s) in `app/src/main.rs` that calls `db_get_snoozed_threads_due`, then calls `unsnooze()` for each due thread. Also check on boot. The undo path already works — `UndoToken::Snooze` calls `unsnooze()`. Gmail has native server-side snooze (thread reappears automatically) but we treat snooze as local-only by design, so the resurface must be local too.
