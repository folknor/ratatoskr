...
- [ ] **bifrost Gmail bulk move to a NON-inbox destination still leaves `SPAM`/`TRASH`** - `move_patch` now strips both when the destination is INBOX (the un-spam fix), but filing a spam or trashed message directly into a user label adds the label and drops `INBOX` only, so Gmail still displays the message in Spam or Trash - it shows a message by its system container regardless of other labels. Same class as the un-spam bug, deliberately not widened when that was fixed.

- [ ] **bifrost Gmail bulk `move_patch` does not special-case the synthetic `archive` container** - Sibling of the un-spam item above, same function. Gmail's `containers_list` synthesises an `archive` container (role `Archive`, native id `archive`) purely so the role table has an Archive entry; `archive` is not a real Gmail label id. The single-object `add_container_patch` special-cases it (drop `INBOX`, add nothing), but the bulk `move_patch` lowers any `MembershipScope::Label` destination straight into `addLabelIds`, so a bulk archive would ask Gmail to apply a nonexistent label. Ratatoskr no longer routes archive that way (`dispatch_target::container_move_plan` composes Gmail archive as an INBOX detach), so this is currently unreachable from us - but the asymmetry between the two patch builders is the actual defect, and any consumer that bulk-moves onto a role-resolved destination hits it.

- [ ] **bifrost `bulk_move` cannot carry a source, so Gmail moves cost a call per message** - `move_patch` is destination-only and `MembershipScope` has no source variant, so ratatoskr composes a Gmail move as `bulk_move` plus a per-id `remove_from_container` (`dispatch_target::gmail_source_detach`) - and `remove_from_container` takes a single `MutationTarget`, so there is no bulk detach to fall back on. Correct but O(n) in messages for exactly the label-model provider whose `batchModify` could express add+remove in ONE request. The durable fix is a source on the bulk move surface (or `move_patch` taking `remove_label_ids`), after which the ratatoskr-side detach can be deleted outright.

- [ ] **bifrost JMAP submission replaces `mailboxIds` wholesale** - `on_success_update_email` now correctly clears the `$draft` keyword, but it still rewrites `mailboxIds` as a whole value, so any unrelated mailbox membership on the submitted message is dropped. Predates the `$draft` fix and was not introduced by it. RFC 8621's own submission example removes Drafts and adds Sent by individual mailbox paths rather than replacing the property.
...
- [ ] **saehrimnir could publish a fixture identity digest for drift detection** - Ratatoskr holds byte-identical copies of some mock fixtures (`gmail-incremental.lua` today) with nothing enforcing that they stay in step. The one-sided fix that crosses no repo boundary: hash the fixture source at load, keep the digest on `Fixture`, and expose `{ name, path, sha256 }` on the test-admin surface, so any consumer holding a copy can assert it is driving what it thinks it is driving. A digest is the right key - the `name` field is already served and says nothing about content.

- [ ] **saehrimnir Gmail archive can produce a message with zero `mailbox_ids`** - `removeLabelIds: ["INBOX"]` can leave an email with no mailbox at all, a state the fixture loader itself rejects at load time. The served mutation surface can therefore reach a shape the fixture format considers invalid.
