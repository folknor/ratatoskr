...
- [ ] **`DebtLedger::is_empty` ignores retained proofs, so "empty" and "absent" are indistinguishable to a backend** (lateral finding from the ledger-serialization side-quest, bifrost-side) - A ledger holding only retained proofs reports empty, and `InMemoryCheckpointStore::get_ledger` returns `Default` for a missing account. Harmless today because a proof is only retained while it can contribute to an open entry or barrier, but that invariant lives in `record_proof` and nothing enforces the coupling; wants at least a comment on `is_empty`. Two siblings from the same pass: `record_barrier`'s update arm refreshes `generation`/`evidence`/`resume_from` but not `domain`/`failure_label` (deliberate idempotence or a staleness gap - unadjudicated), and `record_proof`'s per-ack `proved.retain(...)` sweep is O(proofs x entries) on the accepted-checkpoint hot path (bifrost's own notes list ledger compaction as outstanding).

### Ratatoskr follow-ups from the migration

- [ ] **`AttachmentSource::Inline` / `None` are an unresolved correctness question in `hydrate.rs`** - What remains of the ordinal-pairing item after the pairing itself was retired (2026-09-04: `build_consumer_row` now reads `filename` / `content_id` / `inline` / `size` straight off `MessageAttachment`; the RFC822 re-parse survives only for headers, bodies, and the calendar-payload meeting-invite detection, and the golden inputs carry the structured fields - goldens stayed byte-identical, `bifrost-consumer-*` and the attachment script family re-ran green). The open half: the consumer keys BOTH inline-image dedup and `attachments.content_hash` by blob id, and an `AttachmentSource::Inline(Bytes)` attachment has no blob id - hashing the carried bytes instead is a new code path, and guessing wrong silently corrupts dedup (a hard requirement per AGENTS.md) at runtime rather than at compile time. `attachment_blob` handles `Blob` exactly as before and log-and-skips the other variants, so the gap is visible in logs; settle whether the four mail protocols ever actually return `Inline` before building the byte-hashing path.
