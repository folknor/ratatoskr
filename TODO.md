...
- [ ] **bifrost JMAP `Email/set` can still empty `mailbox_ids`** - Both the full-replace form and the `mailboxIds/<last>: null` patch form can leave a message with no mailbox at all. This is the same hole Gmail had, but the correct fix is the OPPOSITE one: RFC 8621 has no All Mail, so a mailbox-less message is not a representable state and JMAP should REJECT with `invalidProperties` rather than fall back to an archive container the way Gmail legitimately does. Surfaced by the saehrimnir agent while reconciling the Gmail case, and recorded in that repo's own TODO too. IMAP already handles it correctly (EXPUNGE destroys the message) and Graph always writes a non-empty parent, so this is JMAP-only.
...
- [ ] **Consume saehrimnir's fixture identity digest** - The mock now serves `GET /test/fixture/identity` returning `{ name, path, sha256 }`, hashed over the fixture source bytes as read off disk, so it is reproducible with plain `sha256sum`. Nothing on this side checks it. We hold a byte-identical copy of `gmail-incremental.lua` in `crates/app/tests/sync-fixtures/` with nothing keeping the two in step, and any future shared fixture inherits the same risk. A harness-side assertion (or a `brokkr sync` preflight) comparing our copy's digest against the running mock's would turn a silent drift into a failed gate. The mock side is deliberately publish-only - deciding that a mismatch matters is ours.

### Ratatoskr follow-ups from the migration

- [ ] **Delete `gmail_source_detach` in favour of bifrost's `bulk_move_from`** - Gmail moves currently cost one extra request PER MESSAGE: `dispatch_target::gmail_source_detach` issues a per-id `remove_from_container` after the bulk move, because bifrost's `bulk_move` could not carry a source. It now can. `Account::bulk_move_from(targets, destination, source, key)` (bifrost `6fed912`) folds the source into the same Gmail `batchModify`, and its default impl forwards to `bulk_move` for the other three providers, which vacate the source as part of the move anyway. Switching the dispatch onto it makes a Gmail move one request instead of 1+N and deletes the workaround outright. Note bifrost's move read-back guard reconciles membership of the DESTINATION only - it does not re-verify absence from the source - so if we want that asserted it belongs in our own gate.
